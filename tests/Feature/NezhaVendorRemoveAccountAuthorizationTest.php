...
final class StandaloneJsonResponse
{
...
final class StandaloneResponseFactory
{
    public function json(array $data, int $status = 200): StandaloneJsonResponse
    {
        return new StandaloneJsonResponse($data, $status);
    }
}

final class VendorRemoveAccountAuthorizationContract
{
    ...
    {
        return [
            'employee actor guard' => self::employeeGuardChecks($root),
            'isolated employee behavior' => self::isolatedEmployeeBehaviorChecks($root),
            'owner deletion flow' => self::ownerFlowChecks($root),
            'route and realm wall' => self::routeAndRealmChecks($root),
        ];
    }
     ...
     */
    private static function employeeGuardChecks(string $root): array
    {
        $method = self::compact(self::methodSource(self::read($root, self::CONTROLLER), 'remove_account'));
        $bodyStart = strpos($method, '{');
        if ($bodyStart === false) {
            throw new \RuntimeException('remove_account() has no method body');
        }
        ...
        self::add(
            ...
            "remove_account() must start with the authenticated request['vendor_employee'] auth-001/Unauthorized. HTTP 403 guard"
        );
        self::add(
            $checks,
            ! str_contains($body, 'vendorType'),
            'remove_account() must not trust the raw vendorType header'
        );

        $targets = [
            "\$vendor=\$request['vendor'];",
            "Order::where('restaurant_id',\$vendor?->restaurants[0]?->id)",
            '$vendor?->wallet',
        ...
        foreach ($targets as $target) {
            ...
            self::add(
                ...
                "employee 403 guard must precede delete-account target operation: {$target}"
            );
        }
    ...
    /**
     * Invoke the production controller method without a framework bootstrap.
     * The request double throws on every target offset, so reaching tenant-owner
     * data after the employee actor lookup makes this contract fail closed.
     *
     * @return list<array{passed: bool, message: string}>
     */
    private static function isolatedEmployeeBehaviorChecks(string $root): array
    {
        ...
        $request = new class extends \Illuminate\Http\Request
        {
            ...
            public function offsetGet($key): mixed
            {
                ...
                if ($key !== 'vendor_employee') {
                    throw new \RuntimeException("remove_account() crossed into target request offset: {$key}");
                }

                return parent::offsetGet($key);
            }
        };
        $request['vendor_employee'] = 7001;

        $checks = [];
        try {
            $response = (new VendorController)->remove_account($request);
            $status = $response instanceof StandaloneJsonResponse ? $response->status : $response->getStatusCode();
            $data = $response instanceof StandaloneJsonResponse ? $response->data : $response->getData(true);
        ...
        self::add(
            ...
            'employee request must return before tenant owner, restaurant, order, wallet, asset, or delete target access'
        );
...
PHP);
        ...
        if (! function_exists('response')) {
            eval('namespace { function response() { return new \\Tests\\Feature\\StandaloneResponseFactory(); } }');
        }
    }
     ...
     * @return list<array{passed: bool, message: string}>
     */
    private static function ownerFlowChecks(string $root): array
    {
        $method = self::compact(self::methodSource(self::read($root, self::CONTROLLER), 'remove_account'));
        $checks = [];
        $ongoingGuard = "if(Order::where('restaurant_id',\$vendor?->restaurants[0]?->id)->whereIn('order_status',['pending','accepted','confirmed','processing','handover','picked_up'])->count()){returnresponse()->json(['errors'=>[['code'=>'on-going','message'=>translate('messages.user_account_delete_warning')]]],203);}";
        $cashGuard = "if(\$vendor?->wallet&&\$vendor?->wallet?->collected_cash>0){returnresponse()->json(['errors'=>[['code'=>'on-going','message'=>translate('messages.user_account_wallet_delete_warning')]]],203);}";

        self::add($checks, str_contains($method, $ongoingGuard), 'owner ongoing-order guard must retain its exact 203 response');
        self::add($checks, str_contains($method, $cashGuard), 'owner collected-cash guard must retain its exact 203 response');
        ...
        foreach ($orderedOperations as $operation) {
            ...
            self::add(
                ...
                "owner deletion operation is missing or out of order: {$operation}"
            );
            if ($position !== false) {
                $previous = $position;
            }
     ...
     * @return list<array{passed: bool, message: string}>
     */
    private static function routeAndRealmChecks(string $root): array
    {
        ...
        self::add(
            ...
            'DELETE api/v1/vendor/remove-account must remain inside the vendor.api + actch:restaurant_app vendor group'
        );
        ...
        self::add(
            ...
            'missing, invalid, and realm-mismatched headers must not gain an extra owner or employee injection path'
        );
...
if (class_exists(TestCase::class)) {
    final class NezhaVendorRemoveAccountAuthorizationTest extends TestCase
    {
        public function test_remove_account_authorization_contract(): void
        {
            foreach (VendorRemoveAccountAuthorizationContract::checks(dirname(__DIR__, 2)) as $group => $checks) {
...
if (PHP_SAPI === 'cli' && realpath((string) ($_SERVER['SCRIPT_FILENAME'] ?? '')) === __FILE__) {
    $groups = VendorRemoveAccountAuthorizationContract::checks(dirname(__DIR__, 2));
    $tests = count($groups);
    $assertions = 0;
    $failures = [];
