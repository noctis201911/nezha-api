...
- [ ] Codex(USDT 最小权限、reviewer 强制 2FA 与独立复核入口·2026-07-14) 权限与行为代码已在隔离 rollout/backport 分支完成：`payment_address_manage` 负责申请/取消/指定网络紧急暂停，独占 `payment_address_review` 只读 pending/show 并 approve/reject；reviewer 强制 2FA、禁止自批、驳回原因选填，紧急暂停只归 manage。现有 V2 页面已通过 1440/390 浏览器验证，但业主最新裁决为“页面 UI 应修改”，因此 V2 只作可运行原型，不得进入 production backport/部署。后续必须提醒业主让 Fable 基于真实后台出修改方案；新效果图经业主确认后，才可补回正式审核入口 UI。未创建生产角色/管理员，未部署、迁移、初始化或开闸。
