...
- [x] Codex(后台/商家同邮箱隔离·2026-07-15) 已完成应用实现及提交前验证：admins / vendors / vendor_employees 新增、修改、批量导入统一跨表邮箱守卫，忽略大小写与首尾空格；已关闭的网页/App 自助注册仍先过 `toggle_restaurant_registration=0` 关闭门，不开放注册、不改顾客账号。新增回归 5 tests/49 assertions，连同管理员登录边界为 10/166；精确分支完整 Feature 191/1013、Unit 15/46 均通过，10 个变更 PHP 文件语法和 diff check 通过。生产只读复核三组邮箱交集均为 0、开关仍为 0；无迁移、无生产数据写入、未部署。
