...
- [ ] Claude(优惠券重做窗口·B方案对标美团) 进行中: 商家券满减/折扣模板重组+顾客端领取到券包。后端: Vendor/CouponController.php·vendor-views/coupon/{index,edit,partials/_details}.blade.php·新coupon_claims migration+Model·Api/V1领取/我的券包接口+routes。不碰admin-views·不碰顾客端free_delivery显示分支(admin在用)·不碰en/messages.php。(2026-06-25)
...
- [ ] Claude(广告竞价第二期·超管侧A窗口) 进行中: 新增 admin/advertisement 竞价参数页(auction-settings)+广告余额充值页(ad-recharge)。碰共享文件 routes/admin.php(advertisement 组内插 4 路由)+admin _sidebar(广告子菜单加 2 项)+Admin/AdvertisementController.php+新 AdBalanceLogic。用 nzcommit 私有 index + diff 核对，不碰优惠券窗口(vendor 侧)/en messages.php WIP。开关仍默认 0，纯超管 UI 零 live 影响。(2026-07-02)
