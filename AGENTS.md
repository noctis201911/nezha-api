...
- [ ] Claude(超管后台M1窗口) 正在改 admin `resources/views/layouts/admin/partials/_sidebar.blade.php`(牵一发动全身,共享单文件): 按 fable-brief/HANDOFF_admin_M1.md 施工 D1(2257行手工嵌套→PHP配置数组+blade循环,零渲染差异)→D2(§A清单藏死菜单,`@if(false)`/配置数组注释,不删路由)→D3(重排8组新导航)。**不碰**路由/控制器/blade文件本体、不碰L1机制、不碰订单组现有子项(收敛挪M2)。每步独立commit+Playwright截图对比+旧URL curl 200复验。🔔 其它窗口这几天请勿动 `_sidebar.blade.php`(admin),避免物理覆盖;`_header.blade.php` 只动POS购物车图标一处(§A#1),幅度小。(2026-07-07)
