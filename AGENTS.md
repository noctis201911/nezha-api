...
- [x] Codex(USDT production 收尾·2026-07-15) ✅已完成：保留 `dea5dd11` 后发布 2FA 重复提交 419 热修 `95313be`（首次成功后按钮同步锁定，不降低 CSRF/TOTP），再发布 active 开关台账 `7ef91c5` / release `20260715-125017-7ef91c5`，credential→change 两闸保持 `1/1`，开放申请/活跃凭据均 0；治理回归 81 tests/542 assertions、台账契约 9/106、健康门与维护零待办通过。`PAR-001` 从 `/login/admin-employee` 真实登录，nginx 仅 1 次 2FA POST 302 后 pending 200、无第二 POST/419；桌面与 390×844 空队列、刷新、独占侧栏、2FA 真值、无横向溢出及 console 0 error（仅既有 Firebase 告警）通过。取证中废弃一张含凭据临时图并立即轮换验证后台/Basic 两组密码、原子替换 DPAPI，TOTP/恢复码不变；无地址或资金写入。API 回滚锚点 `20260715-122703-95313be`。
