...
- [ ] Claude(本地生活批2·攻略MVP+分享面窗口) 正在改 database/migrations(新nezha_guides) + Models/Guide + Admin/GuideController + Api/V1/GuideController + OgImageController(加guide/merchant方法) + routes(api/v1/api.php,web.php,admin.php) + admin-views/guides + PRELAUNCH_SWITCHES + ADMIN_GUIDE（2026-07-08）
- [ ] Claude(2FA加固窗口·0708) 后台2FA真正强制(0614脚手架已在): AdminMiddleware加2FA硬门堵remember-me绕过 + TwoFactorController置2fa_passed标记 + 待加admin layouts/admin/_header的2FA入口链接 + ADMIN_GUIDE. 进程内验过. 未碰他窗WIP(NotificationController/bootstrap/app.php).
