...
- [x] Codex(USDT 最小权限、reviewer 强制 2FA 与 V2 独立复核入口·2026-07-14) 已在隔离 rollout 分支拆出 `payment_address_manage`（申请/取消/指定网络紧急暂停）与独占 `payment_address_review`（只读 pending/show + approve/reject）；reviewer 未注册 2FA 时只放行 setup/enable/本人密码/退出，注册且会话通过第二因子后仍只放行复核端点/本人密码/退出，禁止关闭 2FA、餐厅 CRUD、旧地址直改、充值/汇率/折扣及 manage 动作，紧急暂停只归 manage。V2 正式页面已按现有后台壳层完成 1440/390 真实浏览器验收，驳回原因选填；未创建生产角色/管理员，未部署、迁移、初始化或开闸。
