...
- [ ] Claude(广告竞价第二期·商家侧 Slice B) 进行中: 商家竞价推广 3 旋钮面板。碰 Vendor/AdvertisementController.php(加 promotion/savePromotion + index 过滤 cpc 行)+新 vendor-views/advertisement/promotion.blade.php+routes/vendor.php(advertisement 组 +2 路由)+vendor _sidebar(+1 项)。总开关仍默认 0，商家配置在开关关/ad_balance=0 时惰性(RecomputeAdAuction 不物化)。tier 低/中/高→出价跟随后台 floor/max_per_click 自动;自助即时(status=approved)。用 nzcommit 私有 index+diff 核对，不碰 en messages.php WIP/优惠券已完成。(2026-07-02)
