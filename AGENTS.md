...
- [ ] Codex(商家订单 UI 隔离浏览器回归·2026-07-14) 在最新 `origin/main` 独立工作树 `/tmp/nezha-merchant-order-ui-browser-regression-20260714` 验证订单分组/计数、关键动作与确认、付款凭证、顾客催单、详情/打印隐私、390×844 操作、刷新/返回、console 与双商家隔离；初始阶段只建立隔离 SQLite 夹具、会话和 QA 工件，证实生产代码缺陷后才追加精确文件认领并最小修改。不部署、不写生产数据/配置、不迁移生产库、不新增产品状态、资金契约或安全边界。
