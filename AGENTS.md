...
- [ ] Codex（渗透测试 production 收口·2026-07-22）正在从 production `e4b733e5` 精确回移图片代理退役、富文本清洗与 Guzzle 安全更新，并同步 Nginx 安全头运维说明；不含 migration/生产数据写入，不访问或修改 Web staging `:3002`。
...
- [ ] Claude(挂牌态总闸+后台入口·2026-07-21) 正在改: 新增 app/CentralLogics/NezhaListing.php + Admin/NezhaListingController + admin-views/nezha-listing/ + 迁移(business_settings 键 nezha_listing_status 默认0); 改 RestaurantLogic/Helpers/ProductLogic/Api·OrderController(2600)/Api·RestaurantController 的 nezha_listing_only 读取点 + routes/admin.php + admin _sidebar.blade + config/nezha_switches.php + docs。不碰资金/退款/结算链路, 不改 status/listing_status 语义, 前端零改动。（2026-07-21）
