...
- [ ] Claude(订单取消机制窗口) 正在做【顾客接单后申请取消 + 商家拒单】后端: 新迁移 orders 加 nezha_cancel_* 5列 + OrderLogic.php(新增 finalize_cancellation 共享收尾helper) + Api/V1/OrderController.php(cancel_request端点+track_order补nezha_cancel meta) + Vendor/OrderController.php(裁决 cancel_request_decision approve|reject + reject_order主动拒单 + 补 status:541 留痕缺口) + Api/V1/Vendor/VendorController.php(同步) + routes(api.php+vendor.php) + order-view.blade.php。已批准触L1边界。不碰他窗WIP(NezhaOrderTimeout/OrderTimeoutSweep/PlaceNewOrder/en messages)，提交只精确add自己文件（2026-06-21）
