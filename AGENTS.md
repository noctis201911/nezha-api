...
- [ ] Codex(部署器 fail-closed 修复·2026-07-14) 正在独立工作树修复 `deploy/nzdeploy-api.sh` 的包准备/迁移/P6 权限锁/队列重启/切换后写入探针失败仍继续问题，并补隔离故障注入；已获授权同步 Git 正本与 `/www/wwwroot/api-deploy/nzdeploy-api.sh` 运行入口，禁止实际部署、migration、release 切换、FPM/队列重启及业务数据写入。
