...
- [ ] Codex(B1 最终候选重封·2026-07-15) 正在将五份 `PRELAUNCH_B1_*` 表单与三份 PRELAUNCH QA/收口正本重封到已回归的 API/Web 最终候选；仅文档与哈希审计，不部署、不迁移、不写 production/shared staging、不发真实通知、不运行备份或 demo `REHEARSE/GO`。
