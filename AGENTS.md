- [x] Codex(USDT 最小权限与 reviewer 强制 2FA·2026-07-14) 已在隔离分支拆出 `payment_address_manage`（申请/取消/紧急暂停）与独占 `payment_address_review`（只读 pending/show + approve）；reviewer 未注册 2FA 时只放行 setup/enable/本人密码，注册后仍只放行复核端点/本人密码，禁止关闭 2FA、餐厅 CRUD、旧地址直改、充值/汇率/折扣及 manage 动作。新增 4 测/69 断言，相关回归合计 55 测/350 断言；未创建生产角色/管理员，未部署、迁移、初始化或开闸，正式独立审核 UI 仍等待真实页面效果图确认。
- [ ] Codex(USDT A＋C 主线移植·2026-07-14) 基于固定 `main=d4fcd8fb` 的独立 worktree 移植后端治理提交 `9e0737a`、`039acca`、`78fafd5`，认领其新增地址状态机/通知/UI/凭据文件及与主线重叠的 `app/Http/Controllers/Api/V1/OrderController.php`、`AGENTS.md`、`docs/compliance/CHANGELOG.md`；只做隔离代码、测试、commit/push，不迁移、不部署、不翻闸、不写生产数据/配置。
- [ ] Codex(商家移动端付款凭证审核入口·2026-07-14) 正在独立 sparse worktree 修复 staging 商家订单详情待审核凭证在 390×844/更窄视口的拒绝入口不可见问题；认领 `resources/views/vendor-views/order/order-view.blade.php`、相关付款凭证契约测试与 QA 正本文档，先复现再改。复用现有审核状态/动作 owner，不新增拒绝语义、状态机、通知时机或后端契约；只授权 staging，不部署 production。
...
- [x] Codex(上线前全平台 QA·2026-07-14) 已完成机器可达深度并判定 **NO-GO**：含测试夹具修复分支的主 QA 基线 Unit 13/29、Feature 177/914 全绿，全订单/退款/保证金回滚、三组各 20 单并发、隔离备份恢复、顾客/商家/后台 Chromium 与 P2 性能均已覆盖；随后最新 `origin/main e3ea7fb` 的新增目标测试 19/143 通过，但精确 main 全量 Feature 180 项仍因夹具修复 `0a87741` 尚未合入而有 9 errors + 1 failure，不得冒充全绿。阻断与人工门详见 `docs/PRELAUNCH_QA_RESULT_20260714.md`。一次性数据库、19090–19095 服务、QA 探针/脚本均已清零；未部署 API、未运行 migration、未切生产 release。普通 H5 bug 的自动升产边界只由前端仓库 `AGENTS.md` 正文拥有，不包含 API、数据库、开关、支付/资金、安全或数据契约变更。
...
- [x] Codex(USDT 地址状态机实现·2026-07-14) ✅基于 75c6e4c 的隔离后端实现已完成：地址凭据/严格链校验、交易级 TOTP 数据库防重放、商家 owner 确认、不同管理员复核、按凭据排空、原子切换、漂移失败暂停、应急定向撤销、旧入口双重封堵、dry-run 初始化/默认 disabled 维护命令、非空审计表拒绝普通 rollback；全部闸默认 0。定向 SQLite :memory: 43 tests/198 assertions、PHP 全文件语法、路由/接线契约和 diff check 通过；仅隔离分支，未运行迁移、未部署、未翻闸、未改生产数据/配置/正式 UI。第二管理员、通知、经确认 UI、迁移演练和逐闸授权仍是上线前置。
- [x] Codex(USDT 安全治理方案·2026-07-14) ✅隔离分支方案已完成：AGENTS.md + docs/USDT_PAYMENT_SECURITY_GOVERNANCE.md + docs/SECURITY_QA_PLAYBOOK.md。已按当前生产运行版本/脏树重核，记录 6 项证据化风险、默认关闭的地址变更状态机、CSP 收敛、DMARC/DNSSEC、仿冒举报及逐项影响/测试/回滚/授权停点；资金规则已于 2026-07-14 获批，迁移/部署/翻闸/DNS/nginx/生产配置/正式 UI 等外部动作仍须分别授权。
