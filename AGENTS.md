- [ ] Codex(USDT 生产上线候选·2026-07-14) 正在独立 worktree 从最新 `origin/main` 集成已批准的地址凭据、立即换址/30 天脱敏、review/manage+强制 2FA 独立入口与 CSP 脱敏报告端；认领相关新增文件及 `OrderController.php`、`routes/api/v1/api.php`、`AGENTS.md`、合规/治理文档。先全回归、加密备份和 migration-only 验收，再按固定 SHA release 部署；开关保持关闭，第二真人 reviewer/现场 2FA、ID12/TRC20 初始化和逐闸启用另受停点约束。
...
- [x] Codex(USDT 最小权限与 reviewer 强制 2FA·2026-07-14) 已在隔离分支拆出 `payment_address_manage`（申请/取消/紧急暂停）与独占 `payment_address_review`（只读 pending/show + approve/reject）；reviewer 未注册 2FA 时只放行 setup/enable/本人密码，注册后仍只放行复核端点/本人密码，禁止关闭 2FA、餐厅 CRUD、旧地址直改、充值/汇率/折扣及 manage 动作。V2 独立审核入口、选填驳回原因、超时外显驳回和商家 best-effort 通知均已完成隔离验证；当时未创建生产角色/管理员，未部署、迁移、初始化或开闸。
