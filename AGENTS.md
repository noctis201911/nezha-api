...
- [x] Codex(商家面板精简·2026-07-13) 已完成隔离分支：shopInfo 删除商业模式/平台佣金/访问网站；shop/edit 隐藏非默认语言页签但保留翻译提交值、删除图标与封面；restaurant-index 隐藏订单类型/其他设置并以无 id 隐藏字段保留原值。Blade 编译 3/3、结构与保值断言 29/29、git diff --check 通过；未部署。
