...
| `nezha_preorder_dispatch_remind_push` | 1 | 预约单叫车提醒推送(**布尔·默认 1 开·平台级**·07 稿·阶段③)。到每单「建议叫车时间」给商家推「该叫车了：N 单待发」摘要一条(防轰炸:摘要合并 + 在场抑制作业台开着不推 + 一个叫车周期最多一条)。关=只停推送、作业台在场横幅照亮。须总闸 `nezha_preorder_status` 同开才发。商家「通知设置」页可调(总闸开才显)。改后须 `Cache::forget('business_settings_all_data')`。⚠️MVP 平台级(每商家粒度为后续)。 | L2 |
