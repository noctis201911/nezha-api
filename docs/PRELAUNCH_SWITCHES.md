...
| `nezha_merchant_video_status` | **1（未签收）** | 本地生活商家页「店内视频」外链卡**总闸**。2026-07-14 production 只读模型核对：商户 12 存 2 条 `v.douyin.com` 记录，但 `nzm-12-vid-1.jpg`/`nzm-12-vid-2.jpg` 均不在 public disk，规范化输出 0，故前端整卡隐藏。当前不是“可见内容等点头”，而是“开关已开但素材不完整”。补封面并批准内容/外跳，或另取精确 Go 回退 0；当前值不等于批准值。见 `fable-brief/HANDOFF_locallife_merchant_video.md`。 | L1-1(纯信息墙)/内容 |
