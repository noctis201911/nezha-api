...
|---|---|---|---|
| `nezha_payment_address_credential_status` | 0 | USDT 付款地址版本凭据总闸。开=已登录顾客打开付款抽屉时，后台只写一条地址展示凭据（不创建订单、不动购物车/优惠券），提交交易哈希时按凭据内地址核验并关联订单；关=现行链路不读取凭据表。🔴 真开前须①修正生产存量严格校验不通过的地址②客户付款抽屉效果图与真实浏览器验收③前后端同批接入 `address_credential_token`④地址切换和旧凭据承认窗口获业主明确批准⑤迁移/影响/回滚方案获批。 | L1-1 邻 |
