...
| `nezha_autooffline_status` | **0** | **改 1（激活）** | 商家长期不确认订单自动停接单：滚动窗口内商家责任超时取消达 N 单且期间无成功接单(不在场)→自动停接单，保护顾客不被继续喂给失联/不响应商家。商家自助一键/运营后台恢复（**无冷却自动恢复**·业主 2026-07-11）。与退款逾期挂起 `nezha_order_suspended` 独立(各用各列，接单闸 OR 两信号)。sweep 每 cron 直接读 DB，翻值下一分钟生效，无需清缓存/USR2。**激活=真实约束商家，先亲测通知真送达 + 阈值确认**。 | L2 |
...
| `nezha_payment_address_change_status` | 0 | USDT 地址受控变更总闸。开后旧后台表单不能直写 USDT；正常变更必须交易级 TOTP → 商家 owner 确认完整地址 → 不同管理员交易级 TOTP → 按未过期凭据排空 → 原子切换。紧急权限只暂停一个商家的一个网络并撤销未消费凭据，不能自批新地址。🔴 真开前须至少 2 个独立 TOTP 管理员、凭据闸稳定、网络状态初始化/对拍、维护命令监控、全链真机验收及逐闸授权。 | L1-1 邻 |
| `nezha_payment_address_change_approval_ttl_min` | 1440 | 地址变更申请等待商家/复核的有效期，默认 24 小时，可调 30–10080 分钟；它只使未完成申请过期，不是资金地址冷静期，也不延长凭据排空。 | L1-1 邻参数 |
...
| `nezha_merchant_video_status` | 0 | 本地生活商家页「店内视频」外链卡**总闸**(档1·全栈上线 dormant)。开=商家录入的抖音/小红书/TikTok/Instagram 视频以封面卡在 H5 商家详情页「店内视频」rail 展示、点击**外跳**对应平台观看(强 CSP 无 iframe·站内不嵌入播放·L1-1 纯信息墙);关=`merchantDetail` 不透出 `video_links`、前端整卡不显。封面强制商家上传(平台抓不到官方缩略图)·每店≤6 条·白名单外域/无封面/超6条 admin 录入即 422。🔴 技术就绪·真开前须①业主看隔离预览截图点头②翻闸后 `php artisan cache:clear`。见 `fable-brief/HANDOFF_locallife_merchant_video.md`。 | L1-1(纯信息墙)/内容 |
