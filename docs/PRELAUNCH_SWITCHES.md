...
| `nezha_preorder_dispatch_lead_min` / `nezha_preorder_window_remind_min` | 30 / 45 | screen05 作业台预约分区参数(**参数非布尔**·后台可调·业主 2026-07-11)。`dispatch_lead_min`=到窗口提醒「建议 X 开始叫车」的固定提前分钟(非实时 ETA·11-7);`window_remind_min`=下一配送窗口起始 ≤ 本值分钟即在作业台顶部提醒并把该窗口分组标「临近」。空/0 回落默认。改后须 `Cache::forget('business_settings_all_data')`。 | L2 |
