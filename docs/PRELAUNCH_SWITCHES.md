...
|---|---|---|---|
| `TELEGRAM_OIDC_ENABLED` + `TELEGRAM_OIDC_ALLOW_NEW_ACCOUNTS`（**env 开关，不在 business_settings**） | **未设置**（config 默认 false；生产 `api-deploy/shared/.env` 里 `TELEGRAM*` 键 0 命中，2026-07-22 实测） | 顾客 Telegram 登录（OIDC，标准授权码 + PKCE，`oauth.telegram.org`；后端已在生产 dormant，前端已于 2026-07-22 合入 main `cc646e5`、总闸关时按钮不渲染故顾客零可见）。**开之前必须齐的前置**：①🔴 **合规记录补齐**——本功能处理 Telegram 已验证手机号、经境外身份提供商建/绑顾客账号，属新增第三方/跨境身份数据流，留存与数据保护侧**已登记**(CHANGELOG 2026-07-21「TG 登录身份表留存豁免书面化」🔴L1-7, 业主 2026-07-20 批; `docs/compliance/data-protection.md` 已列 `user_external_identities`), **但仅覆盖留存, 未覆盖跨境接收方向与隐私政策披露**——该缺口已于 2026-07-22 补齐(见 CHANGELOG 同日条目)；参照 2026-07-01 同类 Telegram 数据流被定为「🔴L1-7 相邻」并要求隐私政策披露的先例，须先补 CHANGELOG 条目 + 核隐私政策 §3 是否已披露 Telegram 为第三方身份提供商（nezha-auditor GATE 2026-07-22 判为配凭据前的**硬前置**）②凭据由业主本人从 @BotFather → Bot Settings → Web Login 取得并填入 `.env`（AI 不经手；`CLIENT_ID` = Bot ID）③在 BotFather 登记生产 Allowed URLs（`https://nezha.am` + `https://api.nezha.am/api/v1/auth/telegram/callback`）④staging 用真 Telegram 账号走完整往返（现零覆盖）⑤`ALLOW_NEW_ACCOUNTS=true`（= 允许 TG 直接注册新账号，牵涉"新用户可能无邮箱"与账号找回）须业主单独拍板，勿与 ENABLED 一起顺手翻。**改 `.env` 后须 `config:clear` + php-fpm 重启（reload 不够）；🔴 禁 `config:cache`。** 步骤单见桌面 `TG登录_生产凭据申请步骤单_20260722.md`。 | L3（前端/后端实现）· 前置项①为 **L1-7 相邻** |
