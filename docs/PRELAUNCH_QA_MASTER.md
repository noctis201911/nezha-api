...
- 本地 R2 代码证据：API `763786c`（功能提交 `d91100d`，锚定当时最新 `origin/main=45a8e8c6b5aeb84bcd7ed93babc7db825b1ac7de`）通过通知聚焦 `40 tests / 154 assertions`、桥接脚本 `4/4`、触及 PHP/JS 语法与 diff 检查；Web `52b8ad5`（离线 build `12e3546`、凭据闸门 `d65bd1c`、容器路由 `17c64e3`、安装实例 `5e7c462`）通过安装实例/容器/凭据闸门 `13/13`、Next `15.5.20` compile-only production build与 `npm audit=0`；商家 Android App `53ddcda`（含 `cac9975`）通过 token hash/轮换实质单测 `4/4`、`assembleDebug`/`lintDebug`，lint 为 `0 errors / 40 warnings`，XML `14/14` 可解析。debug APK 为 4,867,057 B，SHA-256 `B396AEC64B85460057D1F946E27A129676CA1609389654B69FDC41CC0A37ED5D`。
- 上述证据只关闭本地代码语义、编译、静态权限/备份规则、token 生命周期与“不合格构建 fail closed”子门。测试强制 SQLite `:memory:`，未连接真实渠道；Web 普通 build 在缺公共登录凭据时 exit 1，显式 `build:compile-only` 无需手工伪造文件、跳过 analytics 外部读取并 exit 0；Next 的 PostCSS 依赖以同 major `8.5.19` override 替换有漏洞的 `8.4.31`，重新 `npm ci`、13/13、build 与 audit 0 通过；Android `assembleRelease` 在缺 Firebase 与外部签名输入时由 `verifyReleaseReadiness` exit 1。
