...
- 仍开放：相对 `origin/main` 的第三个 migration 文件差异只是在既有 outbox 状态注释中补入 `queued`，无可执行/schema 差异。正式 release signing/Play key custody、Play full-screen intent / `specialUse` 声明及审核未完成；本机无 Java/Android SDK/`adb`/`emulator`，Firebase-enabled APK 未构建；物理 Android/iPhone、D1–D6 与商家 App 未验；FCM 真实收达、邮件/Telegram 命名收件人及真实回执、独立渗透/外部报告仍缺失。production/shared staging 未部署、未写入、未运行 migration，production 继续 **NO-GO**。
