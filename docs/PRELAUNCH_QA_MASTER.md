...
2026-07-15 MVP 阶段优先级裁决（收窄）：仅律师/法律专业方的外部签名、法律书面结论及其追收/验收延期到 MVP 阶段结束后，包括隐私与 USDT/AML 问题中属于法律意见的部分；固定快照与相应空白状态原样保留。会计/税务、非法律 AML 专业核验、独立专业渗透、真实设备/渠道与通知证据、demo 裁决、开关目标及其它内部 owner 工作全部继续，不随法律外签延期。该裁决不表示法律项已签、已通过或获豁免，也不授权任何运行态动作；若拟议动作在公开 production、真实用户/订单/资金、USDT 或真实通知暴露前依法或按现行门禁必须取得法律意见，仍须在该暴露前满足，不能用“MVP”标签替代。
...
- 本地 R2 代码证据：API `763786c`（功能提交 `d91100d`，锚定当时最新 `origin/main=45a8e8c6b5aeb84bcd7ed93babc7db825b1ac7de`）通过通知聚焦 `40 tests / 154 assertions`、桥接脚本 `4/4`、触及 PHP/JS 语法与 diff 检查；当前重新定界候选 `e0bd1dfbfe302d2b5fe9c1149e01e96cfe3656fe` 合并最新三份 owner 正本 `3653d44d5e086c7ccc5de3c5a24e0f5be7476834` 与 R2 `d682fe180796ec8d2fd7294389ca9921d6ec8a1b`，相对 `d682fe1` 只有这三份 Markdown 差异，应用与 migration 文件逐项相同。Web `8094f708c7d39d0a3f7db9048bbbbbb807ce67f2`（父提交 `bb550ec`）在原 17 项门上新增 Service Worker 激活竞态回归，`21/21` 通过；Next `15.5.20` 以真实非生产公共 Firebase 配置 build exit 0，BUILD ID `ofhzpn6YxTjxbjmRdYnP3`。商家 Android App 仍为 `53ddcda`（含 `cac9975`），既有 token hash/轮换实质单测 `4/4`、`assembleDebug`/`lintDebug`、lint `0 errors / 40 warnings`、XML `14/14`；既有 debug APK 4,867,057 B / SHA-256 `B396AEC64B85460057D1F946E27A129676CA1609389654B69FDC41CC0A37ED5D` 早于真实 Firebase 配置，不能证明 App FCM。
- 上述代码证据关闭本地代码语义、编译、静态权限/备份规则、token 生命周期与“不合格构建 fail closed”子门。原测试强制 SQLite `:memory:`；`e0bd1df` 只合并正本文档，当前工作站没有 PHP 运行时，因此没有伪称在该 merge SHA 重跑 PHP，应用树相等性与既有 `40/154` 分开记录。Web 普通 build 的公共配置门仍 fail closed；真实非生产 build 的唯一 warning 是未设置 `NEXT_PUBLIC_BASE_URL` 时既有占位 analytics URL 的 TLS 失败，不是 Firebase 配置失败。Android `assembleRelease` 仍在缺正式外部签名输入时由 `verifyReleaseReadiness` exit 1。
- 2 个本地通知 migration 的**隔离 MySQL 门已关闭**：2026-07-15 16:18–16:20 CEST 在新建的本机 MySQL `8.4.3`、`127.0.0.1:23316`、一次性库 `nezha_notification_r2_isolated` 上，只建立 `users`/旧版 `vendor_device_tokens` 最小 fixture，顺序执行 `up → down → up`。首次 forward 后安装实例表 11 列、1 个 user 外键、2 个业务唯一键；旧商家 token 为 `text` + `char(64)` hash 唯一键，2/2 hash 命中、0 明文、2/2 可解密。rollback 后 2/2 明文恢复、hash 列消失、`varchar(512)` 与旧 token 唯一键恢复，安装实例表删除；再次 forward 的 migration 记录 2/2、hash 2/2、明文 0。最小 fixture 缺 `business_settings` 只产生 provider INFO 日志，命令均 exit 0；最终 mysqld 停止、目录精确删除、23316 无监听。该证据不等于 shared staging/production 已应用。
- 2026-07-15 16:22 CEST 已在仓库外 `C:\Users\Administrator\.nezha-local-qa\notification-r2-20260715` 生成并验证本机非生产标准 Web Push VAPID（public decoded 65 B、private 32 B、public SHA-256 `563A6EC395DB15FDF8E70A9A147E8911DB7B7E4409A3DBA019F9774A781D22DC`）及一次性 PKCS#12 测试 signer（SHA-256 `9AB071A44EAA7165EA8D3CAD4CE8AFE58DDAADEC8D47F9FBA42EFE61DEA2BAF3`）；API/Web public halves一致，原始 PEM 已删除，目录 ACL 仅 SYSTEM 与本机 Administrator。它们未进入 Git，且测试 signer 不是 Play/production release signing。
- 非生产 Firebase 配置子门已关闭：现有项目 `nezha-dummy`（项目号 `1006584491869`）已确认 Cloud Messaging API v1 启用、legacy 禁用，环境标签保持 `Unspecified` 而非伪标 production；现有 Web app 配置与 Web Push certificate 已装入 Web ignored `.env.local`（文件 SHA-256 `597DAE4D6C7A67EADE70CEB1A81D3CA74B6FBA47E6191F2732757CDC46505F81`，Firebase VAPID SHA-256 `B63EF0B3226D9A873DCB262B312315B6149DF51BD4D1032E10603D8742796AF2`）。Android 测试 app `am.nezha.merchant` / `Nezha Merchant Android QA` 已注册，ignored `google-services.json` SHA-256 `A6440806FBA1B8462596C2B461BF45552EECEFDCA09BC47533A79C7304B78310`。仓库外服务账号 JSON SHA-256 `BD5F11202776790342888B894986DC03650C3FD9B9F4AA561552924A8BE0D135`，ACL 仅本机 Administrator/SYSTEM；API ignored `.env` 只保存该外部路径，SHA-256 `952B27AC26892D05113DD0D4F07C8BA18A62A789AD9546620D433BB0E15E5983`。OAuth 200，假 token 与真实 Web FCM token 的 v1 `validate_only=true` 分别得到预期 `INVALID_ARGUMENT` 和 200/name，均未投递消息。
- 真实浏览器子证据：Pixel 10/Android Chrome 仿真下通知权限 `granted`，Firebase messaging worker active、messaging IndexedDB 存在，真实 token 注册路径被本机回环桥接捕获；修复前的 `no active Service Worker` 不再出现。向命名本机测试实例 `QA-D2E-PW-20260715` 受控提交 1 条真实 FCM，Google 返回 200/name，但约 30 秒内 Service Worker 通知列表与前台页面均未观察到唯一 marker，因此只记“服务端接受、无收达回执”，不关闭 FCM 或 D2 真机门。
- 物理 iPhone 标准 Web Push 子门已按单次真实证据关闭：iOS `26.3.1` 实体机在 Safari 安装到主屏幕后注册订阅；非生产 VAPID subject 改为项目 URL `https://nezha.am`，endpoint/audience/public-key/TTL/编码预检通过。唯一受控消息于 `2026-07-15 19:46:53 CEST` 获 Apple HTTP 201，marker SHA-256 `D4CBE310D9D79013C63992062C2CB5A24BE784E2E5419994DEFB6DFBE4CFF7E5`；测试人随后点击明确标注为“我已看到系统通知”的回执，服务端于 `19:48:25 CEST` 记录 1 次收达、发送计数保持 1。仓库外无秘密证据摘要 SHA-256 为 `42673C704A179E3F4F6193C10C36F5625E3C5893B0F71C24E24278F5F2FA706B`。该证据只关闭这一台实体 iPhone 的标准 Web Push 接受与可见收达，不关闭 D1 核心 H5 全流程或任何绝对送达保证。
- 物理 Android 资格核对未关闭 D2：Xiaomi `2409BRN2CC` / Android 14 可通过 ADB WLAN 连接并加载本机 QA 页，但设备没有 Chrome、Google Play services 或 Play Store；Xiaomi Browser 运行态的 Notification API 缺失且 `PushManager` 为 `undefined`。因此它只能继续覆盖核心 H5，不能冒充 Android Chrome + FCM；需另备含 Google Play services 与 Chrome 的实体 Android。截图 SHA-256 `B185A968CC203D85C2F4E8EC91CE963E163D8C033887D9B345D99B6C41DBE6BB`。
- 仍开放：相对 `origin/main` 的第三个 migration 文件差异只是在既有 outbox 状态注释中补入 `queued`，无可执行/schema 差异。正式 release signing/Play key custody、Play full-screen intent / `specialUse` 声明及审核未完成；Android Platform-Tools/ADB 已安装，但本机仍无 Java、完整 Android SDK 或 emulator，Firebase-enabled APK 未构建；D1 核心 H5、合格 D2 Android Chrome + FCM、D3–D6、商家 App、FCM 真实收达、邮件/Telegram 命名收件人及真实回执、独立渗透/外部报告仍缺失。production/shared staging 未部署、未写入、未运行 migration，production 继续 **NO-GO**。
...
| 14 | 通知送达真机（D1 标准 Web Push；D2/商家 Android App FCM；邮件/Telegram 按角色事件） | §0.9 / [[nezha-order-notifications]] | gate | 🟢本地语义/编译·👤真机与真实回执 | 本地 R2 语义/编译、隔离 migrations、非生产配置、实体 iPhone 单次标准 Web Push✅；D1 核心 H5、D2/商家 App FCM、邮件/TG、签名/Play⬜ |
