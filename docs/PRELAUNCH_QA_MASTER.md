...
2026-07-15 MVP 阶段优先级裁决：外部签回材料的追收、重复邮箱/网盘扫描和表单验收从当前 MVP 执行队列移出，MVP 阶段结束后再恢复；固定快照与空白状态原样保留。该裁决只表示延期，不表示已签、已通过或豁免底层门禁，也不授权任何运行态动作。当前工程实现可以继续而无需反复等待 B1 表单；若当前 MVP 要进入公开 production、承接真实用户/订单/资金或启用 USDT、真实通知等暴露面，相应法律/会计/AML、独立专业渗透、真实设备/渠道证据及精确 Go 仍须在对应暴露前满足，不能用“MVP”标签替代。
