...
状态：**业主已于 2026-07-23 批准裁决项 1–6，并授权本任务所需的提交、推送、staging 发布与同范围缺陷修复。代码候选已基于最新 `origin/main` 完成；目标环境 migration、环境开关与 production 发布尚未执行。隔离 MySQL 5.7.44 migration 验证已通过。**

## 1. 已批准目标

- 顾客 H5 最终使用 `api.nezha.am` 的 host-only `HttpOnly; Secure; SameSite=Lax` Cookie。
...
5. JavaScript 不把响应 token 写入 `localStorage`。
...
- Telegram：OIDC exchange、link/complete 等实际返回 token 的成功响应设置 Cookie。
...
- 2026-07-23 隔离 MySQL 5.7.44：目标 migration `up → down → up` 通过；确认 InnoDB、`utf8mb4_unicode_ci`、全部索引及 `users` 外键 `ON DELETE CASCADE`。
- 同一 MySQL 5.7.44 实例：5 会话上限、迁移绝对期取旧 token 剩余寿命、确认后清除 legacy 关联、CSRF 和登出吊销均通过；实例已停机。
- H5 合并最新主线后，9 个相关源测试与 39 个改动 JS/JSX 解析通过；readiness 与 migration 两组编译时开关均完成 Next.js 15.5.21 production build，29 个静态页面生成成功。
- API 定向套件在 rebase 前为 8 tests / 59 assertions 通过；rebase 对实现文件零重叠，前后稳定 patch-id 一致。目标 release 仍须再跑 PHP 运行时验证，不以补丁一致性替代部署后证据。
- 新 Cookie：host-only、`HttpOnly`、`Secure`、`SameSite=Lax`、Path `/`。
...
- 多标签页共享会话、CSRF 稳定，登出后其它标签页重新探测为未登录。
...
production 前还必须取得 `nezha-auditor GATE` 复核与业主明确点头。
