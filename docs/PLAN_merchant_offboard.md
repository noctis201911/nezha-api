...
> 🔴 **当前结论：不进代码**。✅ L1 决策已记入 `INVARIANTS.md`(新增 **L1-8**)+`CHANGELOG`(`926d388`)；✅ 实现设计硬化完成 = `docs/DESIGN_merchant_offboard.md`(`14f0648`)。**剩余门槛**：对 DESIGN 跑一轮 `/debate` 核验 + 你批准前，**不得写实现代码**。
...
2. ✅ **已完成（2026-07-01）**：实现设计硬化 → `docs/DESIGN_merchant_offboard.md`（commit `14f0648`；含 schema DDL / 结算状态机 / 锁+冻结态 / 制裁门 / 来路溯源 / 对账三分支 / 隔离抵扣 + 实测修正红队 E（refund 对直付单 `:1709` 结构性 410）+ §J 6 开放点）。
3. **下一步**：对 DESIGN 跑一轮 `/debate` 核验（§J 6 开放点，尤其加密 PII 跨 vendor 匹配）→ 你批准 → 才写代码。**在此之前不进实现。**
