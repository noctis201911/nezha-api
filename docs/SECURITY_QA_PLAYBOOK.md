...
### 2026-06-22 第四轮 权限分离(P-2 admin module门 + P-3 vendor API employee门) — 已修+已上线(commit 4a7bed1, release 20260622-140458-4a7bed1)
补 admin-employee / restaurant-employee 越权面(轴 C 跨租户/越权后台·商家功能)。当前**非超管 admin=0、vendor_employee=0**,故全为潜伏项——创建受限账号即转真漏洞;现存唯一 admin 是超管(role_id=1 恒 bypass)故零影响。
- **P-2 admin 子系统缺 module 门(🟢潜伏)**:business-settings/file-manager/local-life 路由组原只挂 `admin` guard 无 `module:` 子门 → 任何受限 admin-employee 不论分配模块都能进(改系统/支付/合规开关·文件管理·本地生活冻结/PII)。修:business-settings/file-manager/local-life→`module:settings`,addon-activation→`module:system_addon`,nezha-deposit/merchant-lead→`module:restaurant`。真中间件动态验:受限 admin(modules=['food'])被 order/restaurant 门拒(302)·food 与超管放行。
- **P-3 vendor API 无员工模块门(🟢双重潜伏)**:`VendorTokenIsValid` 对 employee 只设 vendor=owner 不查模块,且 vendor API 组无任何 `module:` 中间件 → 受限员工经 App API 打 owner 级接口(收款账户/提现)绕过 web 模块限制。修:新增 `App\Http\Middleware\VendorApiModulePermission`(owner 无员工上下文恒放行·instanceof 防 query 伪造 / 员工按 employee_role.modules 校验 / JSON 403)+ 别名 `vmodule`(bootstrap/app.php),挂 update-bank-info/request-withdraw/withdraw-method。**双重潜伏:vendor_employee=0 且整个 vendor API 现被 `actch:restaurant_app` 关闭(实测 503,addon 未启用)**。动态验(回滚事务):owner 放行·员工无 settings 模块 403·有则放行。
