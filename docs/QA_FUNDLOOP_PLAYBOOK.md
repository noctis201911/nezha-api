...
| # | 隐患 | 位置 | 级别 | 状态 |
|---|---|---|---|---|
| F-1 | 无 `offline_payments` 行的 offline 单，`confirm_offline_payment` 第~700行 `$order->offline_payments->payment_info` 读 null 属性 → fatal | `OrderLogic::confirm_offline_payment` | 🟡 健壮性 | 2026-06-16 发现，待修(加 null 守卫) |
...
| F-3 | 扣佣/返还对 `deposit_balance` 读-改-写无 `lockForUpdate`，同商家并发送达/退款会 lost-update（少扣一笔佣金） | `create_transaction`/`refund_order` | 🟡 并发正确性 | 待修(行锁或原子 decrement) |
