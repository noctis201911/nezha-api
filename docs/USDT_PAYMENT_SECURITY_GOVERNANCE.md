...
状态：**production backport、migration、CSP Report-Only、有效网络状态初始化、credential/change production canary、原地址恢复、真实独立 reviewer + 强制 TOTP、2FA 重复提交 419 热修和两个功能闸依序启用均已于 2026-07-15 完成。当前 `nezha_payment_address_credential_status=1`、`nezha_payment_address_change_status=1`。**
...
- reviewer 写入前又于 2026-07-15 09:18 UTC 生成加密备份并完成本机解密及 R2 同名对象核验：数据库密文 SHA-256 `a35534c7c1316220147ab7630647b251a31275badb6c016f8dca77dee719f3b0`，文件密文 SHA-256 `c3a1506fe83e73523328a1da689d9bce5046a9d9c40fc8efafd1c47a5b619ede`。
- 最终 change canary 与开闸前又于 2026-07-15 10:24 生成加密备份并完成本机解密及 R2 同名对象核验：数据库密文 SHA-256 `485a633bcd159e9defe3b86b5219bab8f57838da8ef413b620500eb44f6dfd58`，文件密文 SHA-256 `9126ad5574e2421c5cd90516c85079d9f6b253777343e6ddd641feed72bf3858`。
- migration batch 196 已完成；凭据、网络状态、变更和事件表均存在且启用 MySQL `ENCRYPTION="Y"`。两个业务设置键唯一存在，并于 2026-07-15 按 credential → change 顺序启用：`nezha_payment_address_credential_status=1`、`nezha_payment_address_change_status=1`。
...
- 已创建独立生产 reviewer `PAR-001 / 哪吒客服夏莉`：角色 `付款地址复核员` 只含 `payment_address_review`，使用平台 Logo，独立 nginx Basic 用户为 `reviewer_shali`；现场 TOTP 验证成功，生产现有启用 TOTP 的管理员数为 2，恢复码 8 个只以哈希落库。登录密码、Basic 密码和恢复码已拆分写入 owner 工作站的 Windows DPAPI 密文包，二维码、TOTP 临时明文与一次性脚本已从服务器和本机删除。无秘密证据 `/root/nezha-backup/evidence/reviewer-shali-20260715.json` 为 root:root 0600，SHA-256 `6364f6d702468499b403e6940f6c3a618436a96dbfd12d60add96ecd25dafa0c`。
- reviewer 登录入口必须读取 production `data_settings.admin_employee_login_url=admin-employee`，即 `/login/admin-employee`；`/login/admin` 只接受 `role_id=1` 超管，reviewer 在该入口即使邮箱正确也会按设计返回“邮箱不匹配”。桌面凭据助手已同时显示正确登录入口和复核队列地址。
- 最终浏览器取证时发现凭据窗口覆盖页面并进入临时截图，该图立即废弃且不进入验收证据。reviewer 后台密码与 HTTP Basic 密码已于 2026-07-15 13:11 UTC 同批轮换、分别验证生效并原子替换 owner 工作站 DPAPI 密文包；TOTP 与恢复码未暴露、保持不变。无秘密轮换证据 `/root/nezha-backup/evidence/reviewer-shali-credential-rotation-20260715.json` 为 root:root 0600，SHA-256 `a52925aa6b683fd70a00459541e19683ed5ca4094db928073724c0652a4911a3`。
- owner 明确确认 restaurant 12 的 TRC20 钱包由己方控制，并授权只用于 restaurant 6 的临时生产换址测试、不转账、验收后恢复。production 先完成无效 6 位 TOTP 双角色拒绝且地址/版本/申请/事件计数不变；再完成一条独立 reviewer 驳回、一条原地址 A → 受控地址 B 批准，以及 B → A 恢复。restaurant 6 TRC20 版本最终为 3、指纹与 canary 前完全一致，网络 `active`、`pending_change_id=null`、开放申请 0、活跃凭据 0；全过程无链上或资金动作。证据分别为 `/root/nezha-backup/evidence/usdt-invalid-totp-probe-20260715-151318.json`、`usdt-change-reject-20260715-150929.json`、`usdt-change-approve-20260715-151539.json`、`usdt-change-restore-20260715-153207.json`。
- change canary 的逐渠道审计显示：商户站内信/邮件写入或发送返回 `ok`，owner Telegram 返回 `ok`；商户 Telegram/push 因无收件人记录为 `no_recipient`。这里的 `ok` 只证明渠道调用返回成功，不冒充人类已阅读回执。
- production reviewer 登录已真实显示 `PAR-001` 身份和独占侧栏；首次成功的 2FA POST 后旧表单又重复 POST，访问日志依次为 302 成功与 419。根因是提交按钮未一次性锁定，不是验证码先过期。`95313be` 保留 CSRF/TOTP 校验并增加同步重复提交拦截、禁用按钮和“验证中…”反馈；Blade 编译探针及 release 健康门通过。
- 热修后 reviewer 于 2026-07-15 13:03:35 UTC 从 `/login/admin-employee` 完成真实登录；nginx 日志只记录 1 次 `POST /two-factor-challenge` 302，紧接复核队列 GET 200，没有第二次 POST 或 419。桌面视口与 DevTools 精确 390×844 移动视口均显示真实 reviewer 身份、独占侧栏、`本账号 2FA 已启用` 和 0 条空队列，刷新队列后仍为 0；无横向溢出。Console 为 0 error，仅保留项目既有 Firebase 开发版 SDK 告警。
- 发布后 API/Web health、PM2、队列、MySQL/Redis/PHP-FPM/nginx 与 COD=off 均复核正常；后台无 HTTP Basic 时仍返回 401。

### 0.2 已知边界与剩余观察

- 商户 7–11 的 TRC20 当前无效，但 owner 已于 2026-07-15 确认它们是 Claude 生成的示范商家；现存 `_demo_seed_manifest.json` 进一步证明 vendor/restaurant 6–11 都在 demo manifest 内。因此 7–11 的 fail-closed 是 demo 数据质量边界，不再作为真实商户开闸事故上报。restaurant 12 不在该 manifest，但其 TRC20 钱包控制权已由 owner 明确确认。
- 过去 90 天真实 USDT 凭证提交仍为 0；本次 canary 提供的是 2 行受控 production 样本，不是真实自然日流量。凭据表从 0→2→0 时 `data_length=16384`、`index_length=147456` 未跨 InnoDB 分配页，因此只证明小样本不会新增分配页，不能冒充真实日容量或压力测试。owner 曾明确批准“change 继续关闭、先限时开启 credential”的容量采样例外；后续全链 canary 通过并另行授权正式启用后，仍须随自然流量复算。
- production 登录态浏览器已完成超管 1440×1024 零回归，以及 `1/1` 开闸后 reviewer 的桌面空队列、精确 390×844 移动视口、刷新、控制台和 2FA 单次提交终验。服务层的申请、商户确认、独立 reviewer 驳回/批准和恢复也已全部以真实 actor 完成。
- 网络暂停时，顾客端会因凭据签发失败而阻止地址展示和提交；支付方式列表层的主动隐藏仍是后续体验增强，不改变 fail-closed 安全边界。
...
3. **已完成**：创建独立 reviewer、现场绑定强制 TOTP、生成 8 个恢复码并通过 DPAPI 分包交付。
4. **已完成**：使用 owner 控制的 restaurant 12 TRC20 地址临时完成 restaurant 6 的无效 TOTP、驳回、批准与恢复原地址 canary；未转账，最终地址指纹恢复一致。
5. **已完成**：发布 2FA 重复提交热修，并按 credential → change 顺序将两个闸启用为 `1/1`；开闸观察窗 0 个 5xx、0 条新 error log，维护命令为零待办。
6. **已完成**：`PAR-001` 真实浏览器空队列、刷新、390×844 移动视口、console 和 2FA 单次提交终验；异常时仍先关闭 change，再按影响关闭 credential 并清缓存/刷新 worker，必要时把 API `current` 切回 0.1 锚点。不得逆向 migration、删表、删除非本次 demo 的凭据/状态/审计行或写回旧地址。
...
用户 2026-07-14 已批准凭据复用与分层留存；production 已于 2026-07-15 运行该 migration，并在受控换址、恢复和异常码探针通过后按 credential → change 顺序将两个闸开启为 `1/1`。凭据表当前仍为 0 行；真实日签发量尚未产生，因此容量门槛仍未完成。开闸前的限时容量采样严格沿用业主原话“change 继续关闭、先限时开启 credential”；全链 canary 通过后再依另行授权正式启用：
...
3. **不同管理员复核**
   ...
   - 2026-07-15 已建立第二独立管理员 `PAR-001` 并启用 TOTP，生产已具备不同 actor 的组织前提；请求人自批仍必须硬失败，不能提供“紧急自批地址替换”。
...
- reviewer 正式 UI 以 2026-07-14 业主确认的 V3 规格与 18 张基准图为准：复用 production 后台真实顶栏、侧栏和工作台 token，独占 reviewer 侧栏只保留“钱·风控 / 收款地址复核”，隐藏已拍板的首页/洞察与页头四入口；队列只手动刷新或动作后刷新，不轮询，不回显驳回通知渠道结果。页面的 2FA 状态 pill 读取当前管理员真值；未启用时显示琥珀告警、沿用控制器原句说明原因，并禁用交易级动态码与批准/驳回。业主随后批准壳层闭环：reviewer logo 与登录 2FA challenge 成功落到复核队列，账户菜单只保留修改密码/退出，设置页只显示修改密码，页脚 Business setup/Profile/Dashboard 与已启用后的关闭 2FA 入口隐藏；首次绑定、恢复码保存、语言切换、修改密码和退出仍保留。V3 已接入 production release；Fable 唯一阻断证据（移动 401 错误态取景）补拍后按其“补上即转 GO”结论通过，业主于 2026-07-15 终验回复 GO，V3 UI 正式定稿。代码、migration、有效网络初始化和真实 reviewer `PAR-001` 已发布，reviewer 强制 TOTP 已现场启用；受控换址、驳回、恢复、异常动态码探针、空队列桌面/移动视口和 2FA 单次提交均已通过，两个 production 功能闸现为 `1/1`。
...
后端治理基线已推进至 `039acca5ad66765e8cac72f30d4c5001b2d5c168`；本批先在 `codex/usdt-rbac-2fa-20260714-1340` 完成 reviewer 最小权限、强制 2FA、人工驳回与 V2 原型，再在后端隔离分支 `codex/usdt-immediate-switch-retention-20260714` 完成 B 对新付款立即生效、A 凭据自然到期与 30 天脱敏；前端隔离分支 `codex/usdt-credential-reuse-20260714` 完成同一标签页有效凭据复用。业主确认 V3 后，正式 reviewer Blade、pending HTML 分支、同源徽标与独占 reviewer 壳层收敛已接入 `codex/usdt-production-backport-20260714-1830` 候选。MySQL 5.7 临时库已验证新增留存 migration 的 up、重复 up、down、回滚保护和 re-up，临时库已删除且生产库聚合前后不变。以下能力的代码默认值仍保持关闭；production 经既定前置验证后已将 credential/change 运行值开启为 `1/1`：
...
- 初始化命令默认 dry-run，维护命令在闸关闭时只返回 `disabled`；production 当前开闸维护结果为 `expired=0/applied=0/failed=0/pending=0`。普通 migration rollback 拒绝删除非空凭据/状态/审计表。
...
production change canary 与正式启用结论：

- owner 控制的受控有效新地址、商户 owner 确认、不同管理员驳回/批准、恢复原地址和无效 TOTP 拒绝均已完成；地址、版本、开放申请和事件账本已对账。
- 生产通知已记录逐渠道真实返回；商户站内信/邮件与 owner Telegram 为 `ok`，无商户 Telegram/push 收件人时诚实记录 `no_recipient`，不把队列成功冒充人类已读。
- credential/change 已按固定顺序启用为 `1/1`；开闸观察窗无 5xx、无新 error log、无开放申请或活跃凭据，维护命令返回零待办。
- 网络暂停时支付方式列表层的主动隐藏仍是后续体验增强；当前凭据签发失败会安全阻止地址展示与提交。
...
4. **逻辑、V3 UI 与 production backport 已批准并发布**：reviewer 可批准或驳回，二者都需交易级 TOTP；驳回原因选填；超时对外显示“已驳回（超时）”，底层保留 system/expired 审计语义。V3 规格、18 张基准图及 reviewer 壳层闭环均已由业主确认；Fable 对版补证完成，业主于 2026-07-15 终验 GO。production 已创建 `PAR-001` 并现场启用强制 TOTP，真实驳回、批准与恢复动作已在受控 change canary 完成。
5. **部分已批准**：有效 A 凭据自然到期；未消费过期/撤销凭据 30 天后脱敏，已消费凭据随订单/财务证据周期保存。顾客在紧急撤销前后已向旧地址转账时的人工核验、补偿和责任政策仍待单独裁决。
6. **生产授权现状**：加密备份、production backport、migration、API/Web current、网络初始化、独立 reviewer、强制 TOTP、可回滚 demo 数据、credential/change canary、原地址恢复、2FA 热修和两闸依序启用均已按业主授权完成。credential/change 当前为 `1/1`；异常关闸/回滚授权仍有效。nginx/Cloudflare/DNS/邮件、DMARC/CAA/DNSSEC、举报邮箱、`security.txt` 与真实外部举报仍不在本轮授权内，不得外推。
7. **发布口径已执行**：本批以当时 production 为基线精确 backport，并先整合 `dea5dd11` 生产 hotfix，没有把最新 `main` 的其它待发布改动整包上线。API/Web release、migration 和 current 切换已完成且保留 0.1 所列回滚锚点。
...
| DONE | 建立第二独立管理员+TOTP；权限与凭据分包交付 | `PAR-001` 角色独占 `payment_address_review`，现场 TOTP 已启用，Basic/App 密码和恢复码已由 DPAPI 密文包托管 |
| DONE | 后端状态机与顾客地址版本凭据精确 backport、migration、初始化及 credential canary | API/Web release 与回滚锚点已记录；credential canary 清理为 0 |
| DONE | 用 restaurant 6 受控有效新地址完成 change production canary 并恢复 | 无效 TOTP、驳回、批准、恢复、通知审计、最终地址/版本/开放申请对账均完成；无转账 |
| DONE | 两闸正式启用与 2FA 重复提交热修 | `1/1` 顺序启用，观察窗 0 个 5xx/0 新 error log；2FA 首次成功后重复 POST 的 419 已以一次性提交锁修复 |
| DONE | production 最终可见终验 | `PAR-001` 从 `/login/admin-employee` 完成真实登录；桌面与 390×844 空队列、刷新、console、无横向溢出及 2FA 单次提交均通过；废弃含凭据临时图后已轮换两组密码 |
