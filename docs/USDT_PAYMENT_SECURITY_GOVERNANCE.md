...
后端治理基线已推进至 `039acca5ad66765e8cac72f30d4c5001b2d5c168`；本批先在 `codex/usdt-rbac-2fa-20260714-1340` 完成 reviewer 最小权限、强制 2FA、人工驳回与 V2 独立复核入口，再在后端隔离分支 `codex/usdt-immediate-switch-retention-20260714` 完成 B 对新付款立即生效、A 凭据自然到期与 30 天脱敏；前端隔离分支 `codex/usdt-credential-reuse-20260714` 完成同一标签页有效凭据复用。MySQL 5.7 临时库已验证新增留存 migration 的 up、重复 up、down、回滚保护和 re-up，临时库已删除且生产库聚合前后不变。全部功能闸仍默认关闭：
...
4. **逻辑已批准、UI 定稿已撤回**：reviewer 可批准或驳回，二者都需交易级 TOTP；驳回原因选填；超时对外显示“已驳回（超时）”，底层保留 system/expired 审计语义。现有 V2 已实现且通过浏览器验证，但业主最新判断为页面 UI 需要修改，故只作可运行原型；必须后续提醒业主让 Fable 出修改方案，新效果图经业主确认前不得进入 production backport/部署。
5. **部分已批准**：有效 A 凭据自然到期；未消费过期/撤销凭据 30 天后脱敏，已消费凭据随订单/财务证据周期保存。顾客在紧急撤销前后已向旧地址转账时的人工核验、补偿和责任政策仍待单独裁决。
6. **生产授权现状**：加密生产备份已授权并完成；migration、部署/current、第二生产管理员和两个功能闸已获原则授权，但仍因发布范围、UI 重设计、reviewer 真实身份/2FA/Basic 输入及全量地址状态覆盖而处于 NO-GO。真实商户/网络初始化、nginx/Cloudflare/DNS/邮件、DMARC/CAA/DNSSEC、举报邮箱、`security.txt` 与真实外部举报仍未授权，不得由前述授权外推。
7. **发布口径已批准**：以当前 production 为基线精确 backport 本批，不把最新 `main` 的其它待发布改动整包上线。该批准允许准备隔离 backport 候选；当前 UI 重设计停点、reviewer 真实身份/2FA/Basic 输入和真实商户/网络初始化边界仍分别有效，不因发布口径选择而自动放行。

## 8. 实施优先级

| 优先级 | 工作 | 原因 |
|---|---|---|
| P0 | 让 Fable 基于真实后台出独立复核入口修改方案并由业主确认新效果图 | 当前 V2 只作可运行原型；UI 未重新确认前不得进入 production backport/部署 |
