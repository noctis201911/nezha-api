...
状态：**待业主裁决，默认关闭，未部署、未迁移、未修改 DNS/nginx/生产配置**
...
以下建议均涉及资金地址生效规则或外部状态，未获明确授权前只保留在本文：

1. **建议批准**：正常地址变更必须“商家 owner 确认 + 不同管理员复核”，不允许请求人自批；先建立第二管理员并启用 TOTP。
2. **建议批准**：不设任意固定冷静期；按未过期地址凭据自然排空，可配置安全余量但默认 0。
3. **建议批准**：单个管理员经交易级 TOTP 可紧急暂停一个商家的一个网络；改成新地址仍须双角色确认。
4. **需业务裁决**：顾客在凭据被撤销前后已向旧地址转账时的人工核验、补偿和责任政策。
5. **分开授权**：实现地址状态机代码；运行迁移；开启凭据闸；开启地址变更闸；改 CSP/nginx；发布 DMARC；启用 DNSSEC；发布举报邮箱与 `security.txt`。任何一项授权不自动包含其它项。

## 8. 实施优先级

| 优先级 | 工作 | 原因 |
|---|---|---|
| P0 | 建立第二独立管理员+TOTP，批准地址状态机规则后在当前 main 隔离实现 | 当前只有 1 个管理员，安全流程无法正确闭环 |
| P0 | 移植凭据能力并封堵旧地址直写旁路，所有闸默认 0 | 直接降低地址被替换后即时生效的资金风险 |
