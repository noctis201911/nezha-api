...
状态：**资金地址规则已获业主批准；默认关闭的后端隔离实现已完成，仍未部署、未迁移、未翻闸，未修改 DNS/nginx/生产配置或正式 UI**
...
已在 `codex/usdt-security-governance-20260714` 基于实际运行后端版本 `75c6e4c` 完成默认关闭的后端实现：
...
- 生产第二独立管理员及 TOTP；
- 经效果图确认后的管理员申请、商家确认、不同管理员复核 UI；
- 地址变更通知的真实渠道接线、送达结果和失败告警；
- 前端付款页同批获取/提交 `address_credential_token`，并在网络暂停或排空时隐藏对应方式；
...
5. **分开授权**：隔离实现地址状态机代码已批准；运行迁移、部署、开启凭据闸、开启地址变更闸、改 CSP/nginx、发布 DMARC、启用 DNSSEC、发布举报邮箱与 `security.txt` 均未因此获批。任何一项授权不自动包含其它项。

## 8. 实施优先级

| 优先级 | 工作 | 原因 |
|---|---|---|
| P0 | 建立第二独立管理员+TOTP；隔离后端实现已完成，继续补通知与经确认的 UI | 当前只有 1 个管理员，安全流程仍无法正确闭环 |
| P0 | 移植凭据能力并封堵旧地址直写旁路（隔离分支已完成，所有闸默认 0） | 部署、迁移、翻闸仍须分别批准 |
