...
- reviewer 正式 UI 以 2026-07-14 业主确认的 V3 规格与 18 张基准图为准：复用生产后台真实顶栏、侧栏和工作台 token，独占 reviewer 侧栏只保留“钱·风控 / 收款地址复核”，隐藏已拍板的首页/洞察与页头四入口；队列只手动刷新或动作后刷新，不轮询，不回显驳回通知渠道结果。V3 已接入 production backport 候选，仍停在 Fable 对版与业主终验前；未创建或修改生产角色/管理员，未部署、未迁移、未初始化、未翻闸。
...
- reviewer V3 独立入口沿用 production 基线后台壳层；列表仅显示短指纹，详情按需显示完整新旧地址。批准和驳回均要求当前 reviewer 的交易级 TOTP，驳回原因选填；请求人不能自批或自驳，紧急暂停仍只归 `payment_address_manage`。18 张隔离 HTTP 浏览器状态图已产出，当前停在 Fable 对版与业主终验前；logo/账号下拉/页脚及 reviewer 2FA 成功落点是否继续收敛，仍待业主补充裁决，候选不得据此越权修改。
...
4. **逻辑与 V3 UI 已批准**：reviewer 可批准或驳回，二者都需交易级 TOTP；驳回原因选填；超时对外显示“已驳回（超时）”，底层保留 system/expired 审计语义。V3 规格与 18 张基准图已由业主确认，正式 UI 已接入 production backport 候选并产出 18 张实装状态图；当前停在 Fable 对版与业主终验，未获部署授权。
