...
状态：**production backport、migration、CSP Report-Only、有效网络状态初始化、可回滚 credential production canary，以及真实独立 reviewer + 强制 TOTP 已于 2026-07-15 完成；两个业务功能闸当前均为 `0`。change 审批链仍缺受控有效新地址与真实通知回执，因此治理能力尚未全量启用。**
...
- credential 顾客签发/消费路径已用受控 demo 对象完成；change 阶段的独立 reviewer 与 TOTP 已就绪，但仍缺 restaurant 6 某一网络上不同于当前值、确由业主控制的有效新地址，以及商户 owner 的确认会话。通知回执人已指定为超管，但实际送达仍须由超管确认；队列成功不等于邮件/Telegram/push 已送达。
- V3 reviewer production 页面、申请→商户确认→不同管理员批准/驳回和通知真实送达仍未做登录态浏览器终验。`PAR-001` 必须在该链中以自己的登录和 TOTP 成为真实批准/驳回审计 actor，不能由超管代操作。
...
3. **已完成**：创建独立 reviewer、现场绑定强制 TOTP、生成 8 个恢复码并通过 DPAPI 分包交付；生产两闸仍为 `0`。
4. 业主提供 restaurant 6 某一网络的受控有效新地址后，限时开启 change 闸，完成后台 V3 UI 权限隔离、申请、商户确认、独立 reviewer 批准/驳回、超管通知回执与恢复原地址验收。
...
仍未完成、因此不能把 change 闸长期打开：

- restaurant 6 某一网络的受控有效新地址、商户 owner 确认会话，以及申请→确认→独立 reviewer 批准/驳回→恢复原地址的 production 整链验收；
- 生产邮件/Telegram/push 等通知渠道的真实送达验证与失败告警；隔离代码已有 best-effort 调用及逐渠道结果审计，但尚未由已指定的超管确认真实送达，不能表述为已送达；
- 网络暂停时，顾客端支付方式列表层的主动隐藏（当前签发失败会安全阻止展示地址与提交）；
- change 闸限时开启后的逐步观察与最终关闭/启用决策。
...
4. **逻辑、V3 UI 与 production backport 已批准并发布**：reviewer 可批准或驳回，二者都需交易级 TOTP；驳回原因选填；超时对外显示“已驳回（超时）”，底层保留 system/expired 审计语义。V3 规格、18 张基准图及 reviewer 壳层闭环均已由业主确认；Fable 对版补证完成，业主于 2026-07-15 终验 GO。production 已创建 `PAR-001` 并现场启用强制 TOTP；登录态 V3 与真实审批动作仍须在受控 change canary 中终验。
5. **部分已批准**：有效 A 凭据自然到期；未消费过期/撤销凭据 30 天后脱敏，已消费凭据随订单/财务证据周期保存。顾客在紧急撤销前后已向旧地址转账时的人工核验、补偿和责任政策仍待单独裁决。
6. **生产授权现状**：加密备份、production backport、migration、API/Web current、网络初始化、独立 reviewer、强制 TOTP、可回滚 demo 数据和限时 credential canary 均已按业主授权完成。credential/change 两闸当前均关闭。业主已授权后续受控 change canary 与异常关闸/回滚，但仍必须先取得 restaurant 6 的受控有效新地址；通知实际回执由超管确认。nginx/Cloudflare/DNS/邮件、DMARC/CAA/DNSSEC、举报邮箱、`security.txt` 与真实外部举报仍不在本轮授权内，不得外推。
...
| DONE | 建立第二独立管理员+TOTP；权限与凭据分包交付 | `PAR-001` 角色独占 `payment_address_review`，现场 TOTP 已启用，Basic/App 密码和恢复码已由 DPAPI 密文包托管；两闸保持关闭 |
| DONE | 后端状态机与顾客地址版本凭据精确 backport、migration、初始化及 credential canary | API/Web release 与回滚锚点已记录；credential canary 清理为 0，功能闸重新关闭 |
| P0 | 用 restaurant 6 受控有效新地址完成 change production canary | 尚缺实际地址；需验申请、商户确认、独立 reviewer 批准/驳回、超管通知回执、恢复原地址和登录态 V3 浏览器路径 |
