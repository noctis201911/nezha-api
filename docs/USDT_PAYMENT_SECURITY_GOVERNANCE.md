...
当前隔离实现每次签发都会新增一行，尚无复用或清理命令；一旦迁移并开闸，已消费、过期和撤销行都会无限期增长。生产尚未运行该迁移，因此目前没有这张表造成的生产容量压力。拟议留存分层如下，涉及数据删除与财务证据周期，尚未获业务裁决：

- 未消费且已过期/已撤销的凭据：建议 30 天后删除加密地址快照和 secret 哈希，只留不含地址的聚合审计；
- 已消费凭据：随其绑定订单/财务证据的正式留存周期保留，不能先于订单证据删除；
- 签发端复用同一顾客+商家+网络+支付方式尚未过期、未消费的凭据，避免顾客每次刷新页面都新增一行；
...
4. **排空（现行隔离代码；待重新裁决）**
   ...
   - 现行默认未付款超时约 10 分钟，排空通常不超过约 10 分钟；仅当该站点参数被调大时才可能达到硬上限 120 分钟，不是 24 小时。
   - 用户 2026-07-14 对正常换址阻断新凭据的必要性提出质疑。基于“每张凭据已绑定地址快照、网络、商家、用户与订单”的现有证据模型，更小摩擦的候选是：复核完成后立即将新签发切到 B，已签发 A 凭据继续有效到各自过期，新旧地址短时并行；只有怀疑旧地址失陷时才走紧急暂停并撤销未消费凭据。该项改变状态机和资金地址生效规则，必须单独确认后才能替换现行 `draining`，本次 reviewer UI 实现不擅自修改。
...
- V2 已于 2026-07-14 获用户确认：正式入口复用真实总后台顶栏、260px 深色侧栏、卡片、表格与居中弹窗；reviewer 侧栏只保留“钱·风控 / 收款地址复核”。隔离分支据此实现 pending/show/approve/reject 与正式页面，未创建或修改生产角色/管理员，未部署、未迁移、未初始化、未翻闸。
...
后端治理基线已推进至 `039acca5ad66765e8cac72f30d4c5001b2d5c168`；本批在隔离分支 `codex/usdt-ui-ac-20260714` 完成 A＋C 正式 UI/内部通知，在前端隔离分支 `codex/usdt-customer-credential-ui-20260714` 完成顾客地址版本凭据接入，并在 `codex/usdt-rbac-2fa-20260714-1340` 完成 reviewer 最小权限、强制 2FA、人工驳回与 V2 独立复核入口。全部功能闸仍默认关闭：
...
- reviewer V2 独立入口沿用真实后台视觉壳层，只显示复核队列、本人账号与退出；列表仅显示短指纹，详情按需显示完整新旧地址。批准和驳回均要求当前 reviewer 的交易级 TOTP，驳回原因选填；请求人不能自批或自驳，紧急暂停仍只归 `payment_address_manage`。
...
2. **待重新裁决**：仍不设任意固定冷静期；在“正常换址暂停新凭据直到旧凭据排空”与“立即切换新凭据到 B、旧 A 凭据凭快照自然到期并行”之间二选一。现行隔离代码仍是前者，本次不擅自改变。
3. **已批准**：单个管理员经交易级 TOTP 可紧急暂停一个商家的一个网络；改成新地址仍须双角色确认。
4. **已批准**：reviewer 可批准或驳回，二者都需交易级 TOTP；驳回原因选填；超时对外显示“已驳回（超时）”，底层保留 system/expired 审计语义；V2 正式审核入口可以在隔离分支实现。
5. **需业务裁决**：顾客在凭据被撤销前后已向旧地址转账时的人工核验、补偿和责任政策；凭据分层留存周期与清理责任。
6. **分开授权**：业主采用一个明确方案即授权实施该方案逐项列出的隔离代码、测试与分支交付；不自动授权方案未列明的生产或外部动作。运行迁移、部署、创建生产管理员、状态初始化、开启凭据闸、开启地址变更闸、改 CSP/nginx、发布 DMARC/CAA、启用 DNSSEC、发布举报邮箱与 `security.txt` 仍须逐项授权。
