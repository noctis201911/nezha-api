...
- production 登录态浏览器已验证 reviewer 真实身份、独占侧栏和超管 1440×1024 零回归；最终 `1/1` 开闸后的 reviewer 空队列、移动视口以及 2FA 单击锁定仍需在真实会话中补最后一轮可见终验。服务层的申请、商户确认、独立 reviewer 驳回/批准和恢复已全部以真实 actor 完成。
...
6. **下一步**：由 `PAR-001` 在真实浏览器会话补最终空队列/移动视口和 2FA 单击锁定终验；异常时先关闭 change，再按影响关闭 credential 并清缓存/刷新 worker，必要时把 API `current` 切回 0.1 锚点。不得逆向 migration、删表、删除非本次 demo 的凭据/状态/审计行或写回旧地址。
...
| P0 | production 最终可见终验 | `PAR-001` 真实会话补空队列、移动视口与 2FA 单击锁定；不得伪造登录或验证码 |
