...
> 目的：杜绝订单"无限停留在待接单 / 备餐中"。规则**集中在后端一处计算**（`app/CentralLogics/NezhaOrderTimeout.php`），
...
> 其它状态（accepted/handover/picked_up/delivered/canceled/failed/refund_*）不在本规则范围。
...
- 后端：注入 `pending/confirmed/processing` 时间戳为过去 → 跑 `nezha:order-timeout-sweep --dry-run` 验命中、跑实跑验动作 + 幂等（连跑两次第二次 0 动作）+ 边界（阈值前一分钟不触发、后一分钟触发）。
- 前端：用可控测试单（注入旧时间戳）验三色态 + 下一步/退款方式文案渲染，不等真实计时。
