...
| USDT 本单退款地址、不可逆地址指纹、付款/退款链上哈希与新鲜认证结果 | 本单退款路由、安全控制、合规审计 | 高 |
...
- **(2026-07-23)USDT 订单级退款地址凭据**：`nezha_customer_refund_address_credentials` 显式要求 MySQL `ENCRYPTION='Y'`，且 `address_snapshot`、付款交易哈希、付款来源地址和撤销原因再使用 Laravel encrypted cast；数据库以不可逆 `address_fingerprint` 检索地址，以不可逆且唯一的 `submitted_tx_fingerprint` 保证同一链上 TxID 只能形成一份订单退款责任，原始凭据 secret 只存 SHA-256。付款 TxID、来源、合约、精度、实付原子金额、可退 AMD 快照和币种一经本单消费后不可替换。顾客侧只回放本单地址、网络、金额和状态，不返回 `payment_from_address`。本期不采集 KYC 身份材料、不连接钱包、不要求签名，也不做跨订单钱包画像。**当前法律边界**：律师 Q1/Q2 后置期间生产保持 `legal_gate=pending` + `mode=drain`，不接新 USDT；此处记录的是技术数据边界，不声称 customer_attested 已获法律认可。
...
- 链上交易与退款路由记录：合规要求留存 ≥ 5 年（见 `AML-policy.md`）。包括已消费的本单退款地址凭据、付款/退款交易哈希、来源地址、地址指纹、网络/合约/原子金额、策略版本和退款时新鲜认证结果；这些已消费证据豁免短期 PII purge，migration/down 不删除。未消费凭据目前仅以 `expires_at` 失效、不物理删除；具体清理时钟等待律师留存结论后再定。
