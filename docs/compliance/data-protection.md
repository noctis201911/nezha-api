...
**版本**: 1.00 (补充统一邮箱认证 challenge 与 consent 留存边界) 　**更新**: 2026-07-22
...
- **(2026-07-22·代码候选未部署、默认关闭)统一邮箱认证数据最小化**：邮箱 challenge 只保存应用层加密邮箱、不可逆 lookup HMAC、OTP HMAC、浏览器秘密 HMAC、状态与短期时间戳；不保存明文 OTP / 浏览器秘密 / 完成令牌。challenge 与新账号 consent 两表另有 MySQL 表空间 `ENCRYPTION='Y'` fail-closed 校验。验证码前不返回账号存在性；旧未验证邮箱不得只凭 Google 或邮箱 OTP 自动接管。正式部署与翻闸前置见 `CUSTOMER_UNIFIED_AUTH.md`。
...
- 统一邮箱认证 challenge(`customer_email_auth_challenges`,含加密邮箱与短期认证元数据)：`expires_at` 过期满 1 天后由 `nezha:purge-customer-email-auth-challenges` 每 15 分钟整行删除；成功、失败、锁定与未完成记录均在射程内。账号创建 consent(`customer_auth_consents`)保留条款 / 隐私版本、时间、语言、渠道和认证方式；顾客账号删除时外键置空，既不阻塞现有删除路径，也不继续保存可直接关联该顾客的 user id。
