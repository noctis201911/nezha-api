...
> 结论：当前 `confirmed / processing / handover` 三个状态动作没有可供 bot 复用的纯核心调用点，命中 handoff §4.4 的硬停点；在决定是否抽共享服务前，不得写 callback 动作处理器。
...
推荐放宽 handoff 的“现有 Vendor 控制器一行不动”，允许做**纯重构、行为不变**：
...
备选的缩范围方案是：P2.2a 只交付 `confirm_pay`，并把接单/备餐/出餐/拒单/拒收全部降级到商家后台。该方案会改变本包“六动作”的已定目标，不能静默采用。
