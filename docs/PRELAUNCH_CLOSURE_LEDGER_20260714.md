...
- Web `52b8ad5`（含 `12e3546` / `d65bd1c` / `17c64e3` / `5e7c462`）：安装实例/容器/凭据闸门 `13/13`，Next `15.5.20` compile-only build exit 0，BUILD ID `Ql8CzpHk8YY6zDP7wlgpN`；普通 build 缺公共登录凭据时 exit 1，不再手工伪造 ignored 文件；compile-only 跳过 analytics 外部读取，最终无占位 API TLS warning。Next 的 PostCSS 以同 major `8.5.19` override，重新 `npm ci`、13/13、build 及 audit 0 通过，原 2 个 moderate 告警已关闭。
...
- **本轮只关闭**：多安装实例、token rebinding/rotation/logout、App 有界重试/refresh、容器路由，Web 公共凭据/compile-only 构建模式分离，以及编译、实质 token 单测、lint、静态权限、备份排除和 Android release fail-closed 子门。
