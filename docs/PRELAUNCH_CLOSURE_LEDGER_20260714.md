...
- Web `12e3546`（含 `d65bd1c` / `17c64e3` / `5e7c462`）：安装实例/容器/凭据闸门 `13/13`，Next `15.5.20` compile-only build exit 0，BUILD ID `wvNDeWy9Nd_S6bHUpfksY`；普通 build 缺公共登录凭据时 exit 1，不再手工伪造 ignored 文件；compile-only 跳过 analytics 外部读取，最终无占位 API TLS warning。2 个 moderate PostCSS/Next 依赖告警仍记录在案。
