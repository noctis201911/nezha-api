...
- **上线门**：独立报告中无未处理 Critical/High、设备矩阵有逐设备证据、FCM/邮件/TG 各有真实回执；不按内部签字人数计数。
...
| 单一 owner 可负责、证据未齐 | preorder/notification/video 目标值；demo 关联裁决；`utf8mb4` 备份恢复；店 12 条件就绪；物理设备矩阵与真实渠道回执 | 可由同一 owner 承担角色，但每项仍需真实证据和对应精确 Go；当前均未关闭 |
...
3. 单一 owner 先完成无需外部专业判断的决策包；真实通知、真机数据写路径、备份修复/运行、demo、店 12 与开关动作仍分别另取精确 Go。
