...
2026-07-15 MVP 阶段优先级裁决（收窄）：仅律师/法律专业方的外部签名、法律书面结论及其追收/验收延期到 MVP 阶段结束后，包括隐私与 USDT/AML 问题中属于法律意见的部分；相应空白字段保持空白。会计/税务、非法律 AML 专业核验、独立专业渗透、真实设备/渠道与通知证据、demo 裁决、开关目标和其它内部 owner 工作继续推进。延期不是签收、通过或门禁豁免；若公开 production、真实用户/订单/资金、USDT 或真实通知等拟议暴露依法或按现行门禁需要法律意见，仍须在该暴露前满足并另取精确 Go。
...
- **当前事实**：仓库安全自动化不等于独立渗透；D1 核心 H5、合格 Android Chrome、微信/TG WebView 和商家 Android App 未完成矩阵，真实 FCM/邮件/Telegram 无回执。2026-07-15 本地 R2 已补齐代码语义、编译、非生产 Firebase 配置、服务账号鉴权与 FCM v1 消息验证；向本机命名仿真实例的 1 条真实 FCM 获服务端 200/name，但未观察到浏览器 marker，不能当作收达。实体 iPhone/iOS 26.3.1 的主屏幕 Web App 已获得一条标准 Web Push 的 Apple HTTP 201 与用户可见回执，只关闭这一单次子门。
...
- **最小正确动作**：2 个通知 migration 的本机一次性 MySQL `up→down→up`、非生产 Firebase 项目/Web/Android/服务账号配置、FCM v1 `validate_only` 与实体 iPhone 单次标准 Web Push 收达门已经关闭；下一步完成 D1 核心 H5，再补正式 release signing/Play key custody、Play full-screen intent/`specialUse` 声明与必要审核、Firebase-enabled App APK，并用含 Google Play services/Chrome 的实体 Android 及明确命名的邮件/Telegram 接收端做 D2–D6、商家 App、FCM/邮件/Telegram 受控实测。任何 shared staging/production migration 或运行写入仍是另一精确动作包，不由本台账授权。
...
| 目标 | 保证范围 | 证据边界 |
|---|---|---|
| D1 iPhone Safari | 核心 H5；符合平台条件并安装到主屏幕后单列标准 Web Push | 不承诺 FCM；单次实体 iPhone 订阅/Apple 201/用户回执已通过，核心 H5 矩阵仍开放 |
...
- Web `8094f708c7d39d0a3f7db9048bbbbbb807ce67f2`（父提交 `bb550ec`）：在安装实例/容器/公共凭据/通知发布配置闸门上新增 Service Worker active/redundant/timeout 回归，本地矩阵 `21/21`；真实非生产公共配置 build exit 0，BUILD ID `ofhzpn6YxTjxbjmRdYnP3`。Android Chrome 仿真真实浏览器确认通知权限 granted、messaging worker active、messaging IndexedDB、token 获取与注册调用；修复前 `no active Service Worker` 不再出现。原 compile-only、fail-closed、依赖 audit 0 证据保持有效。
- 商家 Android App `53ddcda`（含 `cac9975`）：既有 token hash/轮换实质单测 `4/4`、`assembleDebug`、`lintDebug` 通过；`am.nezha.merchant` 已注册到非生产 Firebase，ignored `google-services.json` 结构/包名/项目号通过，SHA-256 `A6440806FBA1B8462596C2B461BF45552EECEFDCA09BC47533A79C7304B78310`。Android Platform-Tools/ADB 已安装，但当前工作站仍无 Java、完整 Android SDK 或 emulator，未重建 Firebase-enabled APK；旧 debug APK 早于配置，不关闭 App FCM、release signing 或真机门。
- **本轮只关闭**：多安装实例、token rebinding/rotation/logout、App 有界重试/refresh、容器路由，Web 公共凭据/通知公共配置/compile-only 构建模式分离，以及编译、实质 token 单测、lint、静态权限、备份排除和 Web/Android release fail-closed 子门。
- **本轮关闭**：用户精确 Go 后，2 个新增 API 通知 migration 在本机 MySQL `8.4.3` 一次性库完成 `up→down→up`；安装实例表外键/唯一键、商家 token `text`+hash 唯一键、2/2 加密/解密/rollback 恢复均通过，最终进程/端口/目录精确清理。仓库外生成的标准 Web Push VAPID public SHA-256 为 `563A6EC395DB15FDF8E70A9A147E8911DB7B7E4409A3DBA019F9774A781D22DC`；一次性 PKCS#12 测试 signer SHA-256 为 `9AB071A44EAA7165EA8D3CAD4CE8AFE58DDAADEC8D47F9FBA42EFE61DEA2BAF3`，raw PEM 删除、ACL 收紧、均未入 Git。
- **非生产 Firebase 已配置**：`nezha-dummy`（项目号 `1006584491869`）FCM v1 enabled / legacy disabled，环境保持 `Unspecified`；Web ignored `.env.local`、Firebase VAPID、Android app/`google-services.json`、仓库外服务账号和 API ignored `.env` 均通过结构、SHA-256、忽略规则与 ACL 核对。服务账号 OAuth 200，真实 Web token 的 FCM v1 `validate_only` 200/name；没有输出或提交私钥/token。
- **受控 FCM 实发结果**：命名测试实例 `QA-D2E-PW-20260715` 是本机持久 Chrome/Pixel 10 仿真，不是物理 D2。唯一测试消息的 token SHA-256 为 `D80CA0B0C92225D942E8A64BC6EDB2C94D0E4B2B241837F6ED0D4C6C8E1AC427`、marker SHA-256 为 `45321DECE6B568BC237C0EB57004543E2EA2C5A764F33130D315784EB579E9FA`；Google 返回 OAuth 200、FCM 200/name，但约 30 秒内 SW `getNotifications()` 与前台 DOM 均未观察到 marker。结论是“服务端接受、收达未证实”，FCM 签收仍空白，未重发。
- **实体 iPhone Web Push 实发结果**：物理 iPhone/iOS `26.3.1` 的 Safari 主屏幕 Web App 使用标准 Web Push（不是 FCM）；非生产 VAPID subject 为 `https://nezha.am`。唯一受控消息于 `2026-07-15 19:46:53 CEST` 获 Apple HTTP 201，marker SHA-256 `D4CBE310D9D79013C63992062C2CB5A24BE784E2E5419994DEFB6DFBE4CFF7E5`；测试人点击“我已看到系统通知”，服务端于 `19:48:25 CEST` 记录回执，发送/回执为 1/1 且重发被阻断。仓库外无秘密证据摘要 SHA-256 `42673C704A179E3F4F6193C10C36F5625E3C5893B0F71C24E24278F5F2FA706B`。该证据只关闭单次 D1 标准 Web Push 接受与可见收达。
- **实体 Android 资格结果**：Xiaomi `2409BRN2CC` / Android 14 可通过 WLAN ADB 加载 QA 页，但无 Chrome、Google Play services/Play Store；Xiaomi Browser 的 Notification API 缺失且 `PushManager` 为 `undefined`。截图 SHA-256 `B185A968CC203D85C2F4E8EC91CE963E163D8C033887D9B345D99B6C41DBE6BB`。因此它只能用于核心 H5，不能关闭 D2 Android Chrome + FCM。
- **仍开放**：相对 `origin/main` 的第三个 migration 文件 diff 只是既有 outbox 状态注释补 `queued`，无可执行/schema 差异；shared staging/production migration 与运行记录仍为 0。正式 release signing/Play key custody、Play full-screen intent/`specialUse` 声明/审核、Firebase-enabled App APK、完整 Android SDK/emulator、D1 核心 H5、合格 D2 Android Chrome + FCM、D3–D6、商家 App、FCM 真实收达、邮件/TG 命名接收端与真实回执、独立渗透/外部报告仍缺失。当前可验外部签回附件 SHA-256、实名/角色/机构、签署日期时间/时区数量仍全为 0。
...
| 状态 | 本轮结果 | 剩余边界 |
|---|---|---|
| 已关闭（零 production 写入） | 开关/店 12/静态加密事实漂移已校准；预约与异步通知 exact-main 自动语义通过；通知 R2 的多安装实例/token 生命周期/容器路由和 API/Web/App 编译静态子门、隔离 migrations、非生产配置及实体 iPhone 单次标准 Web Push 收达通过；旧 demo 脚本风险已替换为版本化 fail-closed 入口；demo manifest 外关联已精确暴露；视频 stored=2/normalized=2 的技术可见性已核对 | 只关闭列明子项；不关闭 shared/production migration、签名/Play、D1 核心 H5、D2/商家 App FCM、邮件/TG 或六类 NO-GO |
| 已备妥、法律外签 MVP 后恢复、其余继续 | B1 已生成数据裁决表、律师/会计事实包、设备矩阵、渗透范围、真实通知用例和开关签收表；2026-07-15 已冻结/回归候选并重封五份表单 | 当前 MVP 只暂停律师/法律专业方的签名与法律书面结论追收/验收；会计/税务、非法律 AML、渗透、设备/渠道、demo、开关及内部 owner 工作继续。不得发送未授权真实通知、改配置/素材/数据或冒充 owner 签字 |
| 单一 owner 可负责、证据未齐 | preorder/notification/video 目标值；demo 关联裁决；`utf8mb4` 备份恢复；店 12 条件就绪；通知 R2 shared/production migration、签名/Play；D1 核心 H5、合格 D2/商家 App FCM、D3–D6、邮件/TG | 可由同一 owner 承担内部角色，但店 12 还需朋友作为实际经营者参与；实体 iPhone 单次 Web Push 已有证据，其余列明项仍需真实证据和对应精确 Go |
...
3. 单一 owner 先完成无需外部专业判断的决策包；通知 R2 的本机隔离 migration、非生产 Firebase 配置、服务端 FCM 鉴权/消息验证及实体 iPhone 单次标准 Web Push 收达子门已关闭。下一步完成 D1 核心 H5，并取得合格 D2 Android、Firebase-enabled App、邮件/TG 测试接收端。任何 shared staging/production migration、正式 release signing/Play 申报、其余物理设备/真实渠道通知、真机数据写路径、备份修复/运行、demo、店 12 与开关动作仍按各自精确动作包和停点执行。
4. 仅律师/法律专业方的外签与法律书面结论追收/验收延期到 MVP 阶段结束后；会计/税务、非法律 AML 专业核验、独立专业渗透及真实设备/渠道证据继续。若当前 MVP 拟在此之前进入需要法律意见的公开 production、真实订单/资金、USDT 或真实通知暴露，该法律项仍是对应动作的前置门，不能随排期延期。设备测试人与收件人不再被误列为必须的外部人员。
