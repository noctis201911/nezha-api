...
- **最小正确动作**：2 个通知 migration 的本机一次性 MySQL `up→down→up` 门已经关闭；下一步补真实非生产 Firebase 项目/`google-services.json`/Firebase VAPID、正式 release signing/Play key custody、Play full-screen intent/`specialUse` 声明与必要审核，再用专用测试身份做 D1–D6、商家 App、FCM/邮件/Telegram 受控实测。任何 shared staging/production migration 或运行写入仍是另一精确动作包，不由本台账授权。
...
- Web `bb550ec`（含 `52b8ad5` / `12e3546` / `d65bd1c` / `17c64e3` / `5e7c462`）：安装实例/容器/公共凭据/通知发布配置闸门 `17/17`，Next `15.5.20` compile-only build exit 0，BUILD ID `M17jOYkyV0XrbRMHb8o9e`；普通 build 缺公共登录凭据时 exit 1，即使登录凭据模式已开启，缺任一 Firebase/FCM/Web Push 公共值仍在编译前 exit 1 且不输出值；compile-only 跳过 analytics 外部读取，最终无占位 API TLS warning。Next 的 PostCSS 以同 major `8.5.19` override，重新 `npm ci`、17/17、build 及 audit 0 通过，原 2 个 moderate 告警已关闭。
- 商家 Android App `53ddcda`（含 `cac9975`）：token hash/轮换实质单测 `4/4`、`assembleDebug`、`lintDebug` 通过；lint `0 errors / 40 warnings`，XML `14/14`。debug APK 4,867,057 B，SHA-256 `B396AEC64B85460057D1F946E27A129676CA1609389654B69FDC41CC0A37ED5D`；`assembleRelease` 缺 Firebase 与外部签名输入时由 `verifyReleaseReadiness` exit 1。仍未内置 Firebase 配置、未取得 release signing、未真机验证。
...
- **仍开放**：相对 `origin/main` 的第三个 migration 文件 diff 只是既有 outbox 状态注释补 `queued`，无可执行/schema 差异；shared staging/production migration 与运行记录仍为 0。真实非生产 Firebase 项目、Android `google-services.json`、Firebase VAPID、正式 release signing/Play key custody、Play full-screen intent/`specialUse` 声明/审核、`adb`/`emulator`、物理 Android/iPhone、D1–D6 与商家 App、FCM/邮件/TG 真实回执、独立渗透/外部报告仍缺失；Firebase 控制台停在 Google 登录页，没有创建项目。当前可验外部附件 SHA-256、实名/角色/机构、签署日期时间/时区数量仍全为 0。
...
3. 单一 owner 先完成无需外部专业判断的决策包；通知 R2 的本机隔离 migration Go 已执行并关闭该子门。任何 shared staging/production migration、真实 Firebase 配置/正式 release signing/Play 申报、真实通知、真机数据写路径、备份修复/运行、demo、店 12 与开关动作仍分别另取精确 Go。
