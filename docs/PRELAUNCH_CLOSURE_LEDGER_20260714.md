...
- **当前事实**：仓库安全自动化不等于独立渗透；物理 iPhone Safari、Android Chrome、微信/TG WebView 和商家 Android App 未完成矩阵；真实 FCM/邮件/Telegram 无回执。2026-07-15 本地 R2 已补齐代码语义、编译、非生产 Firebase 配置、服务账号鉴权与 FCM v1 消息验证；向本机命名仿真实例的 1 条真实 FCM 获服务端 200/name，但未观察到浏览器 marker，不能当作收达。
...
- **最小正确动作**：2 个通知 migration 的本机一次性 MySQL `up→down→up`、非生产 Firebase 项目/Web/Android/服务账号配置和 FCM v1 `validate_only` 门已经关闭；下一步补正式 release signing/Play key custody、Play full-screen intent/`specialUse` 声明与必要审核、Firebase-enabled App APK，再用物理设备与明确命名的邮件/Telegram 接收端做 D1–D6、商家 App、FCM/邮件/Telegram 受控实测。任何 shared staging/production migration 或运行写入仍是另一精确动作包，不由本台账授权。
...
- 商家 Android App `53ddcda`（含 `cac9975`）：既有 token hash/轮换实质单测 `4/4`、`assembleDebug`、`lintDebug` 通过；`am.nezha.merchant` 已注册到非生产 Firebase，ignored `google-services.json` 结构/包名/项目号通过，SHA-256 `A6440806FBA1B8462596C2B461BF45552EECEFDCA09BC47533A79C7304B78310`。当前工作站无 Java/Android SDK/`adb`/`emulator`，未重建 Firebase-enabled APK；旧 debug APK 早于配置，不关闭 App FCM、release signing 或真机门。
...
- **仍开放**：相对 `origin/main` 的第三个 migration 文件 diff 只是既有 outbox 状态注释补 `queued`，无可执行/schema 差异；shared staging/production migration 与运行记录仍为 0。正式 release signing/Play key custody、Play full-screen intent/`specialUse` 声明/审核、Firebase-enabled App APK、`adb`/`emulator`、物理 Android/iPhone、D1–D6 与商家 App、FCM 真实收达、邮件/TG 命名接收端与真实回执、独立渗透/外部报告仍缺失。当前可验外部签回附件 SHA-256、实名/角色/机构、签署日期时间/时区数量仍全为 0。
...
3. 单一 owner 先完成无需外部专业判断的决策包；通知 R2 的本机隔离 migration、非生产 Firebase 配置与服务端 FCM 鉴权/消息验证子门已关闭。任何 shared staging/production migration、正式 release signing/Play 申报、物理设备/真实渠道通知、真机数据写路径、备份修复/运行、demo、店 12 与开关动作仍按各自精确动作包和停点执行。
