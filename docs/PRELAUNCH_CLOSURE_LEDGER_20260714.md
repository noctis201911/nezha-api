...
- Web `d65bd1c`（含 `17c64e3` / `5e7c462`）：安装实例/容器/凭据闸门 `13/13`，Next `15.5.20` compile-only build exit 0，BUILD ID `6HlUfY_tj_wJp4TO14tLv`；普通 build 缺公共登录凭据时 exit 1，不再手工伪造 ignored 文件。default API TLS warning 与 2 个 moderate PostCSS/Next 依赖告警仍记录在案。
