...
- **仍开放**：2 个新增 API 通知 migration 未应用；相对 `origin/main` 的第三个 migration 文件 diff 只是既有 outbox 状态注释补 `queued`，无可执行/schema 差异。production 运行记录 0；Firebase 配置与 release signing；Play full-screen intent/`specialUse` 声明/审核；物理 Android/iPhone、D1–D6 与商家 App；FCM/邮件/TG 真实回执；独立渗透/外部报告。2026-07-15 16:06 CEST 本机只读范围盘点确认项目路径没有 Firebase/keystore 文件，当前进程没有 Web Firebase/VAPID 与 App 签名输入，且没有可用 `adb`/`emulator`；未读取或输出凭据值。当前可验外部附件 SHA-256、实名/角色/机构、签署日期时间/时区数量仍全为 0。
