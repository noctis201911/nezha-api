...
- **仍开放**：2 个 API 通知 migration 未应用；production 运行记录 0；Firebase 配置与 release signing；Play full-screen intent/`specialUse` 声明/审核；物理 Android/iPhone、D1–D6 与商家 App；FCM/邮件/TG 真实回执；独立渗透/外部报告。当前可验外部附件 SHA-256、实名/角色/机构、签署日期时间/时区数量仍全为 0。
