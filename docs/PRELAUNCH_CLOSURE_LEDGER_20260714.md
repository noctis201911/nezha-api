...
2026-07-15 MVP 阶段优先级裁决：外部签回材料的追收、重复邮箱/网盘扫描和表单验收从当前执行队列移出，MVP 阶段结束后再恢复；在此之前不再把“检查是否有新签回”列为当前动作。固定快照、当前 0 件计数和所有空白字段原样保留。这是延期，不是签收、通过或门禁豁免；工程实现可继续，但公开 production、真实用户/订单/资金、USDT 或真实通知等暴露面仍须在动作前满足其适用的法律/会计/AML、独立专业渗透、真实设备/渠道证据与精确 Go。
...
| 已备妥、不能代签、MVP 后恢复 | B1 已生成数据裁决表、律师/会计事实包、设备矩阵、渗透范围、真实通知用例和开关签收表；2026-07-15 已冻结/回归候选并重封五份表单 | 当前 MVP 不再追签或重复扫描；MVP 后恢复验收。材料仍为 0 件，不得发送真实通知、改配置/素材/数据或冒充 owner 签字 |
...
4. B1 外部签回材料的追收与表单验收延期到 MVP 阶段结束后；若当前 MVP 在此之前要进入公开 production、真实订单/资金、USDT 或真实通知，相应法律/隐私、会计/税务、USDT/AML、独立专业渗透及真实设备/渠道证据仍是对应动作的前置门，不能随排期延期。设备测试人与收件人不再被误列为必须的外部人员。
