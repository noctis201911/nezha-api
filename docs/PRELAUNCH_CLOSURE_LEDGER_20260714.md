...
- API 原已验应用候选 `ab346c42`；B1 包提交 `98efcc2a625e8ba19b068747251d2ed3d66a497d` 只含文档。2026-07-15 06:20 CEST 第二次 fetch 时，`origin/main` 已前移为运行时代码提交 `b14c9c58bee66b59a45bb338f2d742609a3466f3`（parent `98efcc2a`）；无 migration、未改 B1 文档，但尚未纳入本 B1 QA 证据。production 仍为 `e044d34` / `20260714-070255-e044d34`。
...
- 2026-07-15 06:12 CEST 只读复核：`migrate:status` 为 460 Ran / 0 Pending；两个 queue worker online、累计重启各 90，`exit_code=0`、`unstable_restarts=0`，命令仍含 `--max-time=3600`；Redis `PONG`、failed jobs=0。第二次 fetch 后 production→`b14c9c58` migration 文件 diff 仍为 0。上线前仍须重新查 pending/new migration 与运行态。

## B1 外部签收工件

2026-07-14 已从 API 应用基线 `589a5366633f951fc9692810cc2a4c21c553b629`、Web `b4e0ea0f17e3bfc65b3eebe9e645f5334de0faed` 备妥 [B1 包入口](PRELAUNCH_B1_EXTERNAL_SIGNOFF_PACKAGE_20260714.md)，并由文档提交 `98efcc2a625e8ba19b068747251d2ed3d66a497d` 固定：
...
2026-07-15 签回验收结论：本轮可访问的工作区/Desktop/Downloads、Documents 最近 24 小时文件及 API 远端 heads/tags 中，未发现填写后的表单、外部意见、通知回执、设备/渗透报告或签名附件，收到材料 **0 件**。因此外部报告 SHA-256、实名/机构、签署时间/时区可验数量均为 0；不能据此推断“拒绝”，只能判定“尚未收到/未提供”。固定 B1 Git 对象、8 份文档 SHA-256、5 份 manifest 和 2 份既有浏览器证据包均已重算一致，详见 B1 包 §4；这些内部锚点不能代签。`98efcc2a..b14c9c58` 对八份 B1/正本文档 diff=0，所以新增提交不是签回材料。
...
新增候选冻结门：五份 B1 表单记录的 API 应用 SHA 仍是 `589a5366`，当前 `origin/main` 已是 `b14c9c58`。由于收到签名为 0，不存在“已签意见被新 SHA 失效”的问题；但任何最终外部签收前必须先冻结当前候选、对新增运行时代码完成相称回归，并把五份表单重封为同一最终 40-hex。旧 SHA 上的最终“通过”不得接受。
...
| 已备妥、不能代签 | B1 已生成数据裁决表、律师/会计事实包、设备矩阵、渗透范围、真实通知用例和开关签收表；2026-07-15 固定锚点哈希重算一致 | 可访问签回材料仍为 0 件；API 候选已从表单 SHA 漂移，须先冻结/重测/重封；不能发送真实通知、改配置/素材/数据或冒充 owner 签字 |
...
2. 先冻结 API/Web 最终候选，对 `b14c9c58` 及其后续运行时代码做相称回归，并重封五份 B1 表单的固定 SHA；这仍是代码/文档动作，不授权部署或外部写入。
