...
| 8 | 兼容/多端 | 部分通过 | Chromium 390×844/1440×900 无横向溢出；本地容器路由契约已按 D1–D6 固定。实体 iPhone/iOS 26.3.1 已关闭一次主屏幕 Web App 标准 Web Push 收达，但 D1 核心 H5 矩阵仍未完成；实体 Xiaomi Android 14 缺 Chrome/GMS/Play 且运行态无 Notification API/PushManager，只能覆盖核心 H5，不能关闭 D2。Firefox/WebKit 完整矩阵、合格 Android Chrome、微信/TG WebView 和商家 Android App 仍未完成。Chromium 返回为 `navigationType=back_forward`，但 `pageshow.persisted=false`，不能证明 iOS bfcache 命中。 |
...
| 14 | 通知送达 | D1 实体 iPhone 单次标准 Web Push 收达通过；其余真实设备/渠道仍阻断 | 既有退款通知幂等保持；顾客多安装实例、token 轮换/重新绑定/当前实例退出、D1 标准 Web Push、D2 Android Chrome FCM、商家 Android App FCM 与 D3–D6 核心 H5 路由契约已自动化验证。2 个 API migration 在一次性 MySQL 8.4 隔离库完成 `up→down→up` 并精确清理；`nezha-dummy` Web/Android/服务账号配置、真实 Web token 的 FCM v1 `validate_only` 已通过。实体 iPhone/iOS 26.3.1 的唯一受控标准 Web Push 获 Apple 201 并记录用户可见回执；这只关闭该单次 Web Push 子门。命名本机仿真实例的 FCM 虽获 200/name，但无收达 marker；实体 Xiaomi Android 14 缺 Chrome/GMS/Play，不能关闭 D2。shared staging/production 未应用 migration，D1 核心 H5、合格 D2/商家 App FCM、邮件/Telegram、正式 signing/Play 仍缺失，不能称通知维度整体通过。 |
...
- Web：`8094f708c7d39d0a3f7db9048bbbbbb807ce67f2`（父提交 `bb550ec`，含 `52b8ad5`、离线 build `12e3546`、凭据闸门 `d65bd1c`、容器路由 `17c64e3`、安装实例 `5e7c462`）；新增 Service Worker 激活等待和 redundant/timeout 回归，通知本地矩阵由 `17/17` 增为 `21/21`。真实非生产公共配置 build exit 0，BUILD ID `ofhzpn6YxTjxbjmRdYnP3`；唯一额外 warning 是本地未设 `NEXT_PUBLIC_BASE_URL` 时既有占位 analytics URL 的 TLS 失败，不是 Firebase 失败。Android Chrome 仿真真实浏览器确认通知权限、active messaging worker、messaging IndexedDB、token 生成与 API 注册调用，修复前 `no active Service Worker` 警告消失；360 px 视口无横向溢出。原 compile-only、缺配置 fail-closed、PostCSS override 与 audit 0 证据保持有效。
- 商家 Android App：`53ddcda`（含原功能 `cac9975`）；JDK 21 / Android SDK 下既有 token hash/轮换 `4/4` 实质单测、`assembleDebug`、`lintDebug` 通过。`am.nezha.merchant` 已注册到非生产 Firebase，ignored `google-services.json` SHA-256 `A6440806FBA1B8462596C2B461BF45552EECEFDCA09BC47533A79C7304B78310`，Gradle plugin/BOM/messaging 静态接线存在。Android Platform-Tools/ADB 现已安装，但当前机器仍无 Java、完整 Android SDK 或 emulator，故未构建 Firebase-enabled APK；既有 debug APK SHA-256 `B396AEC6...ED5D` 早于该配置，不能作为 FCM 或 Play 候选。正式 release signing 与 Play gate 仍 fail closed。
- 本轮实际关闭：多安装实例、token rebinding/rotation/logout、App 有界重试/refresh、容器路由等本地代码语义，Web compile-only/真实凭据/通知公共配置构建模式分离，以及编译、实质 token 单测、lint、静态权限、备份规则和 Web/Android release fail-closed 子门；另关闭这一台实体 iPhone 的一次标准 Web Push Apple 接受与用户可见收达子门，不外推为 D1 核心 H5 或全平台通知通过。
...
- 非生产 Firebase 配置已关闭：`nezha-dummy` / project number `1006584491869`，FCM v1 enabled、legacy disabled、环境标签 `Unspecified`；Web ignored `.env.local`、Firebase VAPID、Android app/`google-services.json`、仓库外服务账号与 API ignored `.env` 均经结构/哈希/忽略规则验证。服务账号 OAuth 200；假 token 的 `validate_only` 为预期 `INVALID_ARGUMENT`，真实 Web token 的 `validate_only` 为 200/name，均零投递。命名仿真实例 `QA-D2E-PW-20260715` 的 1 条真实 FCM 被 Google 200/name 接受，但约 30 秒内没有浏览器收达 marker，因此只关闭服务端凭据/接口/消息结构，不关闭真实送达。
- 实体 iPhone Web Push：iOS `26.3.1`、Safari 主屏幕 Web App、secure context 与有效订阅成立；非生产 VAPID subject 使用 `https://nezha.am`。唯一受控消息于 `2026-07-15 19:46:53 CEST` 获 Apple HTTP 201，marker SHA-256 `D4CBE310D9D79013C63992062C2CB5A24BE784E2E5419994DEFB6DFBE4CFF7E5`；测试人点击“我已看到系统通知”后，服务端于 `19:48:25 CEST` 记录回执，发送/回执保持 1/1 且禁止重发。仓库外无秘密证据摘要 SHA-256 `42673C704A179E3F4F6193C10C36F5625E3C5893B0F71C24E24278F5F2FA706B`。这关闭单次 D1 标准 Web Push 接受与可见收达，不关闭核心 H5、后台态绝对送达或其他设备。
- 实体 Android 资格核对：Xiaomi `2409BRN2CC` / Android 14 通过 WLAN ADB 加载 QA 页；设备无 Chrome、Google Play services/Play Store，Xiaomi Browser 的 Notification API 缺失且 `PushManager` 为 `undefined`。截图 SHA-256 `B185A968CC203D85C2F4E8EC91CE963E163D8C033887D9B345D99B6C41DBE6BB`；因此只记录“不合格设备”，不关闭 D2/FCM。
- 本轮仍没有关闭：shared staging/production migration 与运行记录；正式 release signing/Play key custody；Play full-screen intent/`specialUse` 声明与审核；Firebase-enabled App APK；完整 Android SDK/emulator；D1 核心 H5、合格 D2 Android Chrome + FCM、D3–D6、商家 App；真实 FCM 收达、邮件/Telegram 命名收件人与回执；外部专业报告。相对 `origin/main` 共出现 3 个 migration 文件 diff，其中 2 个为新增 schema migration，既有 `create_vendor_alert_outbox` 文件的第三处差异只改状态注释、无可执行/schema 变化。
...
6. 通知 R2 已有本地语义/编译、一次性 MySQL `up→down→up`、非生产 Firebase 配置、服务账号 OAuth/FCM v1 `validate_only` 和一次“服务端接受但未观察到收达”的受控 FCM 证据，但没有任何 shared staging/production 应用或运行记录；正式 release signing/Play、Firebase-enabled App APK、物理设备矩阵和真实 FCM/邮件/Telegram 回执仍未关闭。不得把 200/name、隔离 migration 或仓库外测试 key 误记为设备送达或上线证据。
