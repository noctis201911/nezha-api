...
| 14 | 通知送达 | 本地 R2 语义/编译、隔离 migration、非生产 Firebase 配置与 FCM 鉴权通过，真实设备/渠道仍阻断 | 既有退款通知幂等保持；新增顾客多安装实例、token 轮换/重新绑定/当前实例退出、D1 标准 Web Push、D2 Android Chrome FCM、商家 Android App FCM 与 D3–D6 核心 H5 路由契约已自动化验证。2 个 API migration 在一次性 MySQL 8.4 隔离库完成 `up→down→up` 并精确清理；`nezha-dummy` Web/Android/服务账号配置、真实 Web token 的 FCM v1 `validate_only` 已通过。向命名本机仿真实例提交 1 条真实 FCM 得到 200/name，但没有观察到收达 marker；shared staging/production 未应用 migration，正式 signing/Play、物理 D1–D6/商家 App 与 FCM/邮件/Telegram 回执仍缺失，不能称真实送达通过。 |
...
- 商家 Android App：`53ddcda`（含原功能 `cac9975`）；JDK 21 / Android SDK 下既有 token hash/轮换 `4/4` 实质单测、`assembleDebug`、`lintDebug` 通过。`am.nezha.merchant` 已注册到非生产 Firebase，ignored `google-services.json` SHA-256 `A6440806FBA1B8462596C2B461BF45552EECEFDCA09BC47533A79C7304B78310`，Gradle plugin/BOM/messaging 静态接线存在。当前机器无 Java/Android SDK/`adb`/`emulator`，故未构建 Firebase-enabled APK；既有 debug APK SHA-256 `B396AEC6...ED5D` 早于该配置，不能作为 FCM 或 Play 候选。正式 release signing 与 Play gate 仍 fail closed。
