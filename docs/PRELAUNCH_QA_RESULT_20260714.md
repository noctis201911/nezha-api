...
- 生产前端：应用提交 `2f81803`，release `20260714-101004-2f81803`，BUILD `Mguty8CEfSrUIu5FXJ52G`；直付退款阶段前端候选为 `b516b8a`，staging BUILD `n4VGKngOQXDelVRDdK9yN`。首页性能候选为 `6be4453`，已完成隔离 production build，尚未部署 staging 或 production。
...
| 2 | 运维/服务器 | 通过（当前运行态） | nginx/PHP-FPM/MySQL/Redis/PM2/cron/SSL/fail2ban/磁盘/内存检查；Redis `PONG`、failed jobs=0。2026-07-14 最终复核时两个 queue worker 均 online，79 次重启为 `--max-time=3600` 历史累计，每小时正常 SIGINT 重启，不是崩溃循环。 |
...
| 6 | 性能/负载/N+1 | 候选已消除线性增长，staging 负载验收阻断 | 生产旧实现实测查询模型为 `Q(N)=33+16N`：N=1/2/7 分别 49/65/145 SQL。API `6cf7cf9` 在同一生产只读数据、平台配送分支下 N=1/2/7 均为 51 SQL；N=7 完整响应 98,422 B、SHA-256 `231e5a7e451405604f0998592636b8ee097efab7f5a019d7500fa54c014a1b78`，与旧实现逐字节一致。Web `6be4453` 删除首页未消费的 `react-landing-page` SSR 请求，并对按语言 config 增加 60 秒 last-good/single-flight。两仓候选尚未进入 staging，故真实 `/home` p95 仍未复验，不能把隔离结果写成门已绿。 |
...
- 首页性能候选 API `6cf7cf9` / Web `6be4453`：API PHP lint 3/3、Pint 新测试文件和 PHPUnit `2 tests / 17 assertions` 通过；生产只读数据覆盖 N=1/2/7、平台配送费用分支和 N=7 完整 JSON 等价。Web 单飞/语言隔离/TTL 失败回退契约通过，三个触及文件 Prettier 与隔离 Next 15.5.20 production build 通过（BUILD `ncAiXrGUDBtICR8Y56yYI`）。尚未覆盖 staging 真实浏览器与轻量负载、优惠券超过 10 条、订阅餐厅、车辆覆盖边界组合；这些是明确未测项，不得外推为完整发布门通过。
...
2. 先由现有 staging WIP owner 对 Web 10 tracked + 6 untracked、API 37 tracked + 2 untracked 改动做 checkpoint/迁移，再以固定 SHA 安全部署 `6be4453` / `6cf7cf9`；随后复跑 N=1/2/7 查询数、JSON/配送/优惠券兼容矩阵和 `/home` 轻量负载 p95。现有 staging 脚本会对移动 ref 执行 `reset --hard`，API 脚本还无条件 `migrate --force`，当前禁止运行。
...
- 首页性能候选未部署，因此当前回滚仅需分别 `git revert 6cf7cf9` 与 `git revert 6be4453` 形成可审计反向提交；staging 部署后还必须以部署前 checkpoint/构建目录为运行态回滚点。不得通过清理或硬重置现有 staging WIP 来制造“干净环境”。
