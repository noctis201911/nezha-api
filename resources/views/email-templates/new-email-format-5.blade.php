    ...
    <table dir="{{ $site_direction }}" style="width:100%;max-width:500px;margin:0 auto;text-align:center;background:#fff">
        <tr>
            <td style="padding:30px 30px 0">
                ...
                @else
                src='{{ dynamicAsset('assets/admin/img/email-template-img.png') }}'
                @endif
                id="iconViewer" alt="">
        ...
        <tr>
            <td style="padding:0 30px 30px; text-align:left">
                ...
                @else
                <img style="width:120px;display:block;margin:10px auto"  src="{{dynamicAsset('assets/admin/img/favicon.png')}}" alt="public/img">
                @endif
                <span class="privacy">
                    ...
                    @if (isset($social_media))
                        @foreach ($social_media as $social)
                            <a href="{{ $social->link }}" target=”_blank” id="{{ $social->name  }}-check" style="margin: 0 5px;text-decoration:none;{{ (isset($data[$social->name]) && $data[$social->name] == 1)?'':'display:none;' }}">
                                <img src="{{dynamicAsset('assets/admin/img/img/')}}/{{ $social->name }}.png" alt="">
                            </a>
                        @endforeach
                    @endif
                </span>
