        ...
        <table>
            ...
            <tbody>
                @foreach ($data['customers'] as $key => $customer)
                    <tr>
                        <td>{{ $key + 1 }}</td>
                        <td>{{ $customer['f_name'] ? $customer['f_name'] . ' ' . $customer['l_name'] : translate('Incomplete_profile') }}
                        </td>
                        ...
                        <td> {{ \App\CentralLogics\Helpers::format_currency($customer->total_order_amount > 0 && $customer->orders_count > 0 ? ($customer->total_order_amount / $customer->orders_count) : 0) }}
                        </td>
                        <td> {{ $customer->lastOrder ? \App\CentralLogics\Helpers::human_time_format($customer->lastOrder->created_at) : translate('messages.N/A') }}
                        </td>
                        <td> {{ $customer->status ? translate('Active') : translate('Inactive') }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
