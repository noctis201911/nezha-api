{{--
    ...
    哪吒超管M1(D2): $item['hide']=true 时本条目(含其 children,若有)整体不渲染进侧栏,只留一行 HTML 注释(view-source 可见,便于回归/审计),
    路由/控制器/blade 本体不受影响——恢复只需删掉 hide/hide_reason 两个键。
--}}
@if (($item['gate'] ?? ($groupGate ?? true)))
    ...
    @endphp
    @if (!empty($item['hide']))
<!-- 哪吒M1藏: {{ $__label }}{{ !empty($item['hide_reason']) ? ' — ' . $item['hide_reason'] : '' }} -->
    @else
        @php
