...
@section('title', translate('新建集运期次'))
...
<div class="content container-fluid">
    <div class="page-header d-flex justify-content-between align-items-center flex-wrap">
        <h2 class="page-header-title mb-0">{{ translate('新建集运期次') }}</h2>
    ...
    <form action="{{ route('admin.nezha-consolidation-rounds.store') }}" method="post">
        @csrf
                ...
                <div class="row g-3">
                    <div class="col-md-12">
                        ...
                        <input type="text" name="title" class="form-control" required maxlength="191"
                            value="{{ old('title') }}" placeholder="{{ translate('例如：2026 年 8 月首发拼柜') }}">
                    </div>
                    <div class="col-md-4">
                        <label class="input-label">{{ translate('报名截止时间') }}</label>
                        <input type="datetime-local" name="cutoff_at" class="form-control" value="{{ old('cutoff_at') }}">
                    </div>
                    <div class="col-md-4">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="number" step="0.01" min="0" name="min_volume_value" class="form-control"
                            value="{{ old('min_volume_value') }}" placeholder="{{ translate('留空表示不设目标') }}">
                    </div>
                    <div class="col-md-6">
                        ...
                        <select name="min_volume_unit" class="form-control" required>
                            @foreach($unitLabels as $k => $lbl)
                                <option value="{{ $k }}" {{ old('min_volume_unit', 'm3') === $k ? 'selected' : '' }}>{{ $lbl }}</option>
                            @endforeach
                        </select>
        ...
        <div class="card mb-3">
            ...
            <div class="card-body">
                <textarea name="notes" class="form-control" rows="3" maxlength="2000"
                    placeholder="{{ translate('给商家的补充说明（可选）') }}">{{ old('notes') }}</textarea>
            </div>
        </div>

        <div class="text-right mb-4">
            <a href="{{ route('admin.nezha-consolidation-rounds.index') }}" class="btn btn-secondary">{{ translate('取消') }}</a>
            <button type="submit" class="btn btn-primary">{{ translate('创建期次（草稿）') }}</button>
        </div>
    </form>
</div>
@endsection
