...
@section('content')
    <div class="content container-fluid">
        <div class="page-header">
            <h1 class="page-header-title">商家反馈</h1>
            <p class="text-muted mb-0">商家从商家后台「问题反馈」提交的求助/问题。处理后写回复并标「已处理」，商家会收到通知。</p>
        </div>
                ...
                @forelse ($list as $f)
                    ...
                    <div class="border rounded p-3 mb-3">
                        ...
                        <div class="text-muted mb-2" style="font-size:12px;">
                            {{ $rname }} · 类型：{{ $typeLabel }} · {{ $f->created_at }}
                        </div>
                        ...
                        <form method="POST" action="{{ route('admin.vendor-feedback.resolve', $f->id) }}">
                            ...
                            <div class="row g-2 align-items-end">
                                <div class="col-sm-3">
                                    <label class="input-label" style="font-size:12px;">状态</label>
                                    <select name="status" class="form-control form-control-sm">
                                        <option value="open" {{ $f->status == 'open' ? 'selected' : '' }}>待处理</option>
                                        <option value="in_progress" {{ $f->status == 'in_progress' ? 'selected' : '' }}>处理中</option>
                                        <option value="resolved" {{ $f->status == 'resolved' ? 'selected' : '' }}>已处理</option>
                                    </select>
                                </div>
                                <div class="col-sm-7">
                                    <label class="input-label" style="font-size:12px;">回复商家（商家可见）</label>
                                    <input type="text" name="admin_note" class="form-control form-control-sm" maxlength="2000"
                                           value="{{ $f->admin_note }}" placeholder="例如：已核对，本月佣金无误，明细见…">
                                </div>
                                <div class="col-sm-2">
                                    <button type="submit" class="btn btn-sm btn-primary w-100">保存</button>
                                </div>
                            </div>
                        </form>
                    </div>
                @empty
                    <p class="text-center text-muted py-4 mb-0">该状态下暂无反馈。</p>
                @endforelse
                {!! $list->links() !!}
            </div>
        </div>
