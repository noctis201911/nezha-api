        ...
        <thead class="thead-light">
            <tr>
                ...
                @if(!$hide_source_column)
                    <th class="border-0">
                        {{ ($type ?? 'order') === 'subscription' ? translate('messages.Restaurant') : translate('messages.Source') }}
                    </th>
                @endif
        ...
        <tbody id="set-rows">
            @foreach($transactions as $k => $t)
                <tr>
                    ...
                    @if(!$hide_source_column)
                        <td>
                            <div class="mb-1">{{ $t['source'] ?? $t['restaurant'] ?? '' }}</div>
                            @if(isset($t['source_type']))
                                <div
                                    class="badge text-{{ $t['source_type'] == 'Restaurant' ? 'warning bg-warning' : ($t['source_type'] == 'Delivery Man' ? 'info bg-info' : 'success bg-success') }} bg-opacity-10 rounded-lg font-medium px-2">
                    ...
                    <tr class="collapsing-tr d-none bg-light2">
                        ...
                        <td colspan="{{ $hide_source_column ? '3' : '4' }}" class="pr-0">
                            @if($isOrderTransaction)
                                ...
                                @else
                                    ...
                                    @if(isset($t['breakdown']['express_charge']) && $t['breakdown']['express_charge'] > 0)
                        ...
                        <td class="text-right pl-0">
                            @if($isOrderTransaction)
                                @if($hide_source_column)
                                    <div class="mb-2">
                                        {{ \App\CentralLogics\Helpers::format_currency($t['breakdown']['order_sales'] ?? 0) }}</div>
                                    <div class="mb-2">+
                            ...
                            @else
                                ...
                                @else
                                    ...
                                    <div class="mb-2">+
                                        {{ \App\CentralLogics\Helpers::format_currency($t['breakdown']['product_discount'] ?? 0) }}</div>
                                    <div class="mb-2">+
