@php
    $show_restaurant_select = $show_restaurant_select ?? true;
    ...
    <form action="">
        <div class="__bg-F8F9FC-card">
            <div class="row g-3 date-filter-wrapper">
                @if($show_restaurant_select)
                <div class="col-lg-6">
                    <label for="" class="input-label text-capitalize">
                        {{ translate('messages.Select_Restaurant') }}
                    </label>
                    <select name="restaurant_id" id="restaurant_id" class="form-control js-select2-custom">
                        <option value="all" {{ $restaurant_id == 'all' ? 'selected' : '' }}>{{ translate('messages.All_Restaurants') }}</option>
                        @foreach($restaurants as $restaurant)
                            <option value="{{ $restaurant->id }}" {{ $restaurant_id == $restaurant->id ? 'selected' : '' }}>{{ $restaurant->name }}</option>
                        @endforeach
                    </select>
                </div>
                @else
                ...
                @endif
                <div class="{{ $show_restaurant_select ? 'col-lg-6' : 'col-lg-12' }}">
                    <label for="" class="input-label text-capitalize">
...
<div class="card card-body mb-20">
    ...
    <div id="restaurant_earning_summary"></div>

    <h4 class="mb-3">{{ translate('messages.Earnings_Breakdown') }}</h4>
    <div id="restaurant_earning_breakdown"></div>

    <h4 class="mb-3">{{ translate('messages.Expenses_Breakdown') }}</h4>
    <div id="restaurant_expense_breakdown"></div>
</div>

<div class="card h-100 mb-20">
    <div class="card-header border-0 d-block pb-0">
        <h3 class="mb-1 text-title">{{ translate('messages.Restaurant Earnings Trend') }}</h3>
...
<div class="card card-body">
    ...
    <div class="d-flex justify-content-between align-items-center gap-2 flex-wrap border-0">
        ...
        <div class="search--button-wrapper justify-content-end">
            <form id="restaurant-transaction-search-form">
                <!-- Search -->
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            function fetch_all_data() {
                fetch_data('restaurant_earning_summary', '{{ $summary_url }}');
                fetch_data('restaurant_earning_breakdown', '{{ $breakdown_url }}');
                fetch_data('restaurant_expense_breakdown', '{{ $expense_url }}');
            ...
            function fetchTransactions(page = 1) {
                ...
                let url = "{{ $transactions_url ?? route('admin.report.restaurant-earning-transactions') }}";
            ...
            $(document).on('click', '.transaction-tab', function(e) {
                ...
                if (currentTransactionType === 'subscription') {
                    placeholder = "{{ translate('Search By Txn ID or Restaurant Name') }}";
                } else{
            ...
            $('#restaurant-transaction-search-form').on('submit', function(e) {
                e.preventDefault();
            ...
            $(document).on('click', '#export-excel', function() {
                ...
                let url = "{{ $transactions_export_url ?? route('admin.report.restaurant-earning-export') }}";
                url += "?filter=" + filter + "&from=" + from + "&to=" + to + "&restaurant_id=" + restaurant_id + "&type=" + currentTransactionType + "&search=" + search + "&export_type=excel";
                location.href = url;
            });
            ...
            $(document).on('click', '#export-csv', function() {
                ...
                let search = $('#datatableSearch_').val();
                let url = "{{ $transactions_export_url ?? route('admin.report.restaurant-earning-export') }}";
                url += "?filter=" + filter + "&from=" + from + "&to=" + to + "&restaurant_id=" + restaurant_id + "&type=" + currentTransactionType + "&search=" + search + "&export_type=csv";
                location.href = url;
            });
