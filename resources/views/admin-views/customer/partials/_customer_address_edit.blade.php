...
@endphp
<div class="modal fade" id="addressEdit__modal" data-backdrop="static" data-keyboard="false" tabindex="-1">
            ...
            <div class="modal-body pt-0 pb-0">
                <form action="{{ route('admin.customer.update-customer-address', $customer->id) }}" class="mt-lg-4 mt-3 pb-2" method="post">
                    ...
                    <div class="scrolling-area-modal">
                        <div>
                            <input type="hidden" name="address_id" id="address_id" value="{{ data_get($address, 'id') }}">
                            <h3 class="modal-title text-capitalize mb-2 flex-grow-1">{{ translate('Edit Address -') }} {{ data_get($address, 'address_type') }} </h3>
                                ...
                                <div class="row g-2">
                                    <div class="col-md-6">
                                        ...
                                        <select required name="address_type" id="" class="custom-select">
                                            <option {{ $address && $address['address_type'] == 'home' ? 'selected' : '' }}
                                                value="home">{{ translate('messages.home') }}</option>
                                            <option {{ $address && $address['address_type'] == 'office' ? 'selected' : '' }}
                                                value="office">{{ translate('messages.office') }}</option>
                                            <option {{ $address && $address['address_type'] == 'other' ? 'selected' : '' }}
                                                value="other">{{ translate('messages.other') }}</option>
                                        </select>
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <input id="contact_person_number" type="tel" class="form-control"
                                            name="contact_person_number" pattern="^\+[1-9][0-9]{6,14}$"
                                    ...
                                    <div class="col-md-12">
                                        ...
                                        <textarea required id="address" name="address" class="form-control" cols="30" rows="3"
