...
public static function scopeApplyDateFilter($query, $filter, $from = null, $to = null, $column = 'created_at')
{
    return $query->when(isset($from) && isset($to) && $filter === 'custom', function ($query) use ($from, $to, $column) {
        ...
        ->when($filter === 'this_month', function ($query) use ($column) {
            return $query->whereMonth($column, now()->format('m'))
    ...
    public function scopeApplyPreviousDateFilter($query, $filter, $from = null, $to = null, $column = 'created_at')
    {
        return $query
            ->when($filter === 'custom' && $from && $to, function ($q) use ($from, $to ,$column) {
                $days = \Carbon\Carbon::parse($from)->diffInDays(\Carbon\Carbon::parse($to)) + 1;

                $previousFrom = \Carbon\Carbon::parse($from)->subDays($days);
                $previousTo   = \Carbon\Carbon::parse($from)->subDay();

                return $q->whereBetween($column, [$previousFrom, $previousTo]);
            })

            ->when($filter === 'this_year', function ($q) use ($column) {
                return $q->whereYear($column, now()->subYear()->year);
            })

            ->when($filter === 'this_month', function ($q) use ($column) {
                return $q->whereYear($column, now()->year)
                        ->whereMonth($column, now()->subMonth()->month);
            })

            ->when($filter === 'previous_year', function ($q) use ($column) {
                return $q->whereYear($column, now()->subYears(2)->year);
            })

            ->when($filter === 'this_week', function ($q) use ($column) {
