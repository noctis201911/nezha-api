...
class Subscription extends Model
{
    ...
    public function log()
    {
        return $this->hasOne(SubscriptionLog::class, 'subscription_id', 'id')->latest();
    }
    ...
    {
        if ($this->relationLoaded('pause')) {
            $today = now()->format('Y-m-d');
            return $this->pause->filter(function ($pause) use ($today) {
                    return $pause->from <= $today && $pause->to >= $today;
                })->count() > 0;
        }

        return (bool)$this->pause()->whereDate('from','<=', now()->format('Y-m-d'))
