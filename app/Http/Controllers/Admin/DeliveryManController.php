...
class DeliveryManController extends Controller
{
    ...
    {
        $key = explode(' ', $request['search']);
        $dm = DeliveryMan::with(['reviews','shifts'])->where('type', 'zone_wise')->where(['id' => $id])->first();
        if ($tab == 'info') {
    ...
    public function store(Request $request)
    {
        ...
        // Sync shifts using the pivot table
        $dm->shifts()->sync($shifts);
    ...
    public function update(Request $request, $id)
    {
        ...
        $delivery_man->save();
        $changes = $delivery_man->shifts()->sync($request->shifts ?? []);


        if($delivery_man->fcm_token && (count($changes['attached']) || count($changes['detached']) || count($changes['updated']))) {
            $data = Helpers::makeDataForPushNotification(title: translate('Shift updated'), message: translate('Your shift has been updated'), orderId: '', type: 'delivery_nam_shift_update', orderStatus: '');
        ...
        try {
            if ($request->status == 0) {
                ...
                if ($notification_status?->mail_status == 'active' && config('mail.status') && $mail_status == '1') {
                    Mail::to($delivery_man?->getRawOriginal('email'))->send(new \App\Mail\DmSuspendMail('suspend', $delivery_man['f_name']));
                }
            } else {
                ...
                if ($notification_status?->mail_status == 'active' && config('mail.status') && $mail_status == '1') {
                    Mail::to($delivery_man?->getRawOriginal('email'))->send(new \App\Mail\DmSuspendMail('unsuspend', $delivery_man['f_name']));
                }
            }
        } catch (\Exception $ex) {
            info($ex->getMessage());
        ...
        try {
            ...
            if ($request->status == 'approved') {
                if ($notification_status?->mail_status == 'active' && config('mail.status') && Helpers::get_mail_status('approve_mail_status_dm') == '1') {
                    Mail::to($delivery_man?->getRawOriginal('email'))->send(new \App\Mail\DmSelfRegistration('approved', $delivery_man->f_name.' '.$delivery_man->l_name));
                }
            } else {
                ...
                if ($notification_status?->mail_status == 'active' && config('mail.status') && Helpers::get_mail_status('deny_mail_status_dm') == '1') {
                    Mail::to($delivery_man?->getRawOriginal('email'))->send(new \App\Mail\DmSelfRegistration('denied', $delivery_man->f_name.' '.$delivery_man->l_name));
                }
            }
        } catch (\Exception $ex) {
            info($ex->getMessage());
