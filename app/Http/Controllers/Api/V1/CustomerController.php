...
class CustomerController extends Controller
{
    ...
    public function set_default_address(Request $request)
    {
        ...
        $addressExists = CustomerAddress::where('user_id', $userId)
            ->where('id', $addressId)
            ->exists();

        if (!$addressExists) {
            return response()->json(['message' => translate('messages.Address_not_found!!!')], 403);
        }
        ...
        return response()->json([
            'message' => translate('messages.Address_set_as_default')
        ], 200);
    }
