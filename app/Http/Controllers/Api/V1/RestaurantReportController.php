...
/**
 * 顾客举报商家(餐厅)。
 * POST /api/v1/customer/restaurant/{restaurant_id}/report  (middleware: auth:api，仅登录可举报)
 *
 * 仅登录可举报(2026-06-28 用户拍板, 与 local-life 帖/评价举报禁匿名口径一致, 防匿名滥用)。
 *
 * 作用域/鉴权:
 *   - 举报人 = 当前登录用户(auth:api 保证非空)，user_id 服务端取定、绝不信 body → 防伪造他人身份(IDOR)。
 *   - restaurant_id 必须真实存在, 否则 404; vendor_id 由餐厅服务端派生, 不信 body。
 *
 * 防刷(参考 locallife reportPost + nudge Cache):
 *   1. 待处理去重: 同用户对同店已有 pending → 友好 200, 不重复落库。
 *   2. 短时限频: 同用户 + 同店 10 分钟内只受理一次(Cache)。
...
class RestaurantReportController extends Controller
{
    ...
    public function store(Request $request, $restaurant_id)
    {
        ...
        // ── 防刷 2 ── 短时限频(同用户 + 同店 10 分钟)
        $freshKey = 'nezha_restaurant_report_u' . $userId . '_' . $restaurant->id;
        ...
        RestaurantReport::create([
            ...
            'user_id'       => $userId,
            'guest_id'      => null,                      // 仅登录, 无游客(列保留 nullable 供未来扩展)
