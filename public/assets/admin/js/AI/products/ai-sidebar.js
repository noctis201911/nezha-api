            ...
            success: function (response) {
                ...
                $('.use-title-btn').off('click').on('click', function (e) {
                    ...
                    if ($productNameInput.length) {
                        $productNameInput[0].value = title;
                        $productNameInput[0].dispatchEvent(new Event('input'));
...
$(document).on('click', '#analyzeImageBtn', function () {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            const el = document.getElementById('default_name');
            if (el) {
                el.value = response.data.title;
...
function waitForDescriptionAndContinue(lang, $button, $imageRemoveButton, $chooseImageBtn,requestType) {
    ...
    const checkDescription = setInterval(function () {
        ...
        if (hasContent || checkCount >= maxChecks) {
            ...
            setTimeout(() => {
                const desc = descriptionField.val();
                if (typeof CKEDITOR !== 'undefined' && CKEDITOR.instances['description-' + lang]) {
                    CKEDITOR.instances['description-' + lang].setData(desc);
                }
                if (descriptionField.length) {
                    descriptionField[0].dispatchEvent(new Event('input'));
                }
...
$(document).on("change", "#image-input2", function (e) {
