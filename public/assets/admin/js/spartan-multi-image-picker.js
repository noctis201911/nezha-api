...
(function ($) {
    ...
    $.fn.spartanMultiImagePicker = function (options) {
        ...
        var defaults = {
            ...
            maxCount: '',
            maxFileSize: '',
            allowedExt: 'webp|png|jpg|jpeg|gif',
            onAddRow: function () { },
        ...
        function addRow(settings, el) {
            ...
            var acceptAttribute = 'image/*';
            if (settings.allowedExt && settings.allowedExt !== '') {
                acceptAttribute = '.' + settings.allowedExt.split('|').join(',.');
            }
            ...
            var template = `<div class="${groupClassName} spartan_item_wrapper" data-spartanindexrow="${count}" style="margin-bottom : 20px; ">` +
                ...
                `<input class="form-control spartan_image_input" accept="${acceptAttribute}" data-spartanindexinput="${count}" style="display : none"  name="${fieldName}" type="file">` +
                `</label> ` +
                `</div>` +
                `</div>`;
        ...
        function loadImage(settings, input, parent) {
            ...
            if (input.files && input.files[0]) {
                ...
                var regex = new RegExp(`(.*?)\\.(${allowedExt})$`, 'i');

                // Capture if this slot was empty before we started processing
                // We check if the preview image is NOT visible.
                var is_empty_slot = !$(parent).find('img[data-spartanindeximage="' + index + '"]').is(":visible");

                if (regex.test(file_select_name) || allowedExt == '') {

                    if ((maxFileSize == '') || (maxFileSize != '' && file_select.size <= maxFileSize)) {

                        if (file_select.type.match('image.*')) {
                            ...
                            // Logic for documents (PDF, Excel, etc.) - SYNCHRONOUS
                            console.log('SPARTAN: Document detected (Sync). Index:', index);

                            var $fileUpload = $(parent).find('.file_upload');
                            var $placeholder = $(parent).find('img[data-spartanindexi="' + index + '"]');
                            var $previewImg = $(parent).find('img[data-spartanindeximage="' + index + '"]');
                            var $removeBtn = $(parent).find('a[data-spartanindexremove="' + index + '"]');
                            var $viewBtn = $(parent).find('a[data-spartanindexview="' + index + '"]');

                            $fileUpload.addClass('has-image');
                            ...
                            var docIcon = settings.docImage || '/public/assets/admin/img/document.svg';

                            $previewImg.attr('src', docIcon).css('display', 'block');
        ...
        return this.each(function () {
            ...
            if ($(this).data('spartan-initialized')) return;
            $(this).data('spartan-initialized', true);

            var that = this;

            // Only add initial row if maxCount is not specified OR maxCount > 0
